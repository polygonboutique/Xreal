...
/// includes ===================================================================
// system -------------------------------------------------------------------
#include <ctype.h>		//Tr3B -  gcc3 -Wall  toupper fix
// qrazor-fx ----------------------------------------------------------------
#include "cl_local.h"
...
void 	Key_SetBinding(int keynum, char *binding)
{
	...
	{
		Com_Free(keybindings[keynum]);
		keybindings[keynum] = NULL;
	}
			
	// allocate memory for new binding
	l = strlen (binding);	
	nw = (char*)Com_Alloc(l+1);
	strcpy (nw, binding);
	nw[l] = 0;
...
============
*/
void 	Key_WriteBindings(VFILE *stream)
{
	for(int i=0; i<256; i++)
	{
		if(keybindings[i] && keybindings[i][0])
			VFS_FPrintf(stream, "bind %s \"%s\"\n", Key_KeynumToString(i), keybindings[i]);
	}
}
...
void 	Key_Event(int key, bool down, int time)
{
	...
	if (key == K_ESCAPE)
	{
		...
		switch(cls.key_dest)
		{
			...
			default:
				Com_Error(ERR_FATAL, "Key_Event: bad cls.key_dest");
		}
		return;
	}
