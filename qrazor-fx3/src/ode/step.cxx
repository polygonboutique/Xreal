...
static void Multiply2_p8r (vec_t *A, vec_t *B, vec_t *C,
...
static void MultiplyAdd2_p8r (vec_t *A, vec_t *B, vec_t *C,
			      int p, int r, int Askip)
{
  int i,j;
  vec_t sum,*bb,*cc;
...
static void Multiply0_p81 (vec_t *A, vec_t *B, vec_t *C, int p)
{
  int i;
  dIASSERT (p>0 && A && B && C);
  vec_t sum;
...
static void MultiplyAdd0_p81 (vec_t *A, vec_t *B, vec_t *C, int p)
{
  int i;
  dIASSERT (p>0 && A && B && C);
  vec_t sum;
...
static void MultiplyAdd1_8q1 (vec_t *A, vec_t *B, vec_t *C, int q)
{
  int k;
  vec_t sum;
  dIASSERT (q>0 && A && B && C);
...
static void Multiply1_8q1 (vec_t *A, vec_t *B, vec_t *C, int q)
{
  int k;
  vec_t sum;
  dIASSERT (q>0 && A && B && C);
			     ...
			     dxJoint * const *_joint, int nj, vec_t stepsize)
{
  ...
  ALLOCA(vec_t,I,3*nb*4*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (I == NULL) {
      UNALLOCA(joint);
      dMemoryFlag = d_MEMORY_OUT_OF_MEMORY;
...
#endif
  ALLOCA(vec_t,invI,3*nb*4*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (invI == NULL) {
      UNALLOCA(I);
      UNALLOCA(joint);
  ...
  for (i=0; i<nb; i++) {
    ...
    dMULTIPLY2_333 (tmp,(vec_t*)body[i]->mass._I,(vec_t*)body[i]->R);
    dMULTIPLY0_333 (I+i*12,(vec_t*)body[i]->R,tmp);
    // compute inverse inertia tensor in global frame
    dMULTIPLY2_333 (tmp,(vec_t*)body[i]->invI,(vec_t*)body[i]->R);
    dMULTIPLY0_333 (invI+i*12,(vec_t*)body[i]->R,tmp);
...
#endif
  int nskip = dPAD (n6);
  ALLOCA(vec_t, invM, n6*nskip*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (invM == NULL) {
      UNALLOCA(ofs);
      UNALLOCA(info);
  ...
  ALLOCA(vec_t,fe,n6*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (fe == NULL) {
      UNALLOCA(invM);
      UNALLOCA(ofs);
  ...
  ALLOCA(vec_t,v,n6*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (v == NULL) {
      UNALLOCA(fe);
      UNALLOCA(invM);
  ...
  ALLOCA(vec_t,vnew,n6*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (vnew == NULL) {
      UNALLOCA(v);
      UNALLOCA(fe);
...
#endif
    ALLOCA(vec_t,cfm,m*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (cfm == NULL) {
      UNALLOCA(c);
      UNALLOCA(vnew);
...
#endif
    ALLOCA(vec_t,lo,m*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (lo == NULL) {
      UNALLOCA(cfm);
      UNALLOCA(c);
...
#endif
    ALLOCA(vec_t,hi,m*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (hi == NULL) {
      UNALLOCA(lo);
      UNALLOCA(cfm);
...
#   ifdef TIMING
    dTimerNow ("create J");
#   endif
    ALLOCA(vec_t,J,m*nskip*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (J == NULL) {
      UNALLOCA(findex);
      UNALLOCA(hi);
...
#   ifdef TIMING
    dTimerNow ("compute A");
#   endif
    ALLOCA(vec_t,JinvM,m*nskip*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (JinvM == NULL) {
      UNALLOCA(J);
      UNALLOCA(findex);
    ...
    int mskip = dPAD(m);
    ALLOCA(vec_t,A,m*mskip*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (A == NULL) {
      UNALLOCA(JinvM);
      UNALLOCA(J);
...
#   endif
    ALLOCA(vec_t,tmp1,n6*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (tmp1 == NULL) {
      UNALLOCA(A);
      UNALLOCA(JinvM);
    ...
    ALLOCA(vec_t,rhs,m*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (rhs == NULL) {
      UNALLOCA(tmp1);
      UNALLOCA(A);
...
#   endif
    ALLOCA(vec_t,lambda,m*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (lambda == NULL) {
      UNALLOCA(rhs);
      UNALLOCA(tmp1);
...
#endif
    ALLOCA(vec_t,residual,m*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (residual == NULL) {
      UNALLOCA(lambda);
      UNALLOCA(rhs);
...
#   endif
    ALLOCA(vec_t,L,m*mskip*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (L == NULL) {
      UNALLOCA(rhs);
      UNALLOCA(tmp1);
...
#endif
    memcpy (L,A,m*mskip*sizeof(vec_t));
...
#endif
    memcpy (lambda,rhs,m * sizeof(vec_t));
			     ...
			     dxJoint * const *_joint, int nj, vec_t stepsize)
{
  ...
  ALLOCA(vec_t,cforce,nb*8*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
  if (cforce == NULL) {
    UNALLOCA(ofs);
    UNALLOCA(info);
...
#   ifdef TIMING
    dTimerNow ("create J");
#   endif
    ALLOCA(vec_t,J,2*m*8*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (J == NULL) {
      UNALLOCA(findex);
      UNALLOCA(hi);
...
#   ifdef TIMING
    dTimerNow ("compute A");
#   endif
    ALLOCA(vec_t,JinvM,2*m*8*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (JinvM == NULL) {
      UNALLOCA(J);
      UNALLOCA(findex);
    ...
    for (i=0; i<nj; i++) {
      ...
      vec_t body_invMass = body[b]->invMass;
      vec_t *body_invI = invI + b*12;
      vec_t *Jsrc = J + 2*8*ofs[i];
      vec_t *Jdst = JinvM + 2*8*ofs[i];
...
#   ifdef TIMING
    dTimerNow ("compute rhs");
#   endif
    ALLOCA(vec_t,tmp1,nb*8*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (tmp1 == NULL) {
      UNALLOCA(A);
      UNALLOCA(JinvM);
...
//    dTimerNow ("factorize A");
//#   endif
//    vec_t *L = (vec_t*) ALLOCA (m*mskip*sizeof(vec_t));
//    memcpy (L,A,m*mskip*sizeof(vec_t));
//#   ifdef FAST_FACTOR
...
//    dTimerNow ("compute lambda");
//#   endif
//    vec_t *lambda = (vec_t*) ALLOCA (m * sizeof(vec_t));
//    memcpy (lambda,rhs,m * sizeof(vec_t));
//    dSolveCholesky (L,lambda,m);
...
#   endif
	...
	vec_t data1[8],data2[8];
	Multiply1_8q1 (data1, JJ, lambda+ofs[i], info[i].m);
	vec_t *cf1 = cforce + 8*b1->tag;
...
#ifdef COMPARE_METHODS
  ALLOCA(vec_t,tmp, ALLOCA (nb*6*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (tmp == NULL) {
      UNALLOCA(cforce);
      UNALLOCA(ofs);
...
void dInternalStepIsland (dxWorld *world, dxBody * const *body, int nb,
			  dxJoint * const *joint, int nj, vec_t stepsize)
{
