...
static inline vec_t myDot (vec_t *a, vec_t *b, int n)
{
  vec_t sum=0;
  while (n > 0) {
    sum += (*a) * (*b);
...
# ifdef ROWPTRS
  ...
  if (do_fast_row_swaps) {
    ...
    ALLOCA (vec_t,tmprow,n * sizeof(vec_t));
    ...
    memcpy (tmprow,A[i1],n * sizeof(vec_t));
    memcpy (A[i1],A[i2],n * sizeof(vec_t));
    memcpy (A[i2],tmprow,n * sizeof(vec_t));
    UNALLOCA(tmprow);
  }
...
# else
  ...
  if (i1 > 0) {
    memcpy (tmprow,A+i1*nskip,i1*sizeof(vec_t));
    memcpy (A+i1*nskip,A+i2*nskip,i1*sizeof(vec_t));
    memcpy (A+i2*nskip,tmprow,i1*sizeof(vec_t));
  }
  for (i=i1+1; i<i2; i++) {
    tmp = A[i2*nskip+i];
...
static void swapProblem (ATYPE A, vec_t *x, vec_t *b, vec_t *w, vec_t *lo,
			 vec_t *hi, int *p, int *state, int *findex,
...
static void checkFactorization (ATYPE A, vec_t *_L, vec_t *_d,
				int nC, int *C, int nskip)
{
  ...
  // compare A2 and A3
  vec_t diff = A2.maxDifference (A3);
  if (diff > 1e-8)
...
struct dLCP {
  int n,nub,nskip;
  vec_t *Adata,*x,*b,*w,*lo,*hi;	// LCP problem data
  ATYPE A;				// A rows
  ...
  vec_t Aii (int i);
  vec_t AiC_times_qC (int i, vec_t *q);
  vec_t AiN_times_qN (int i, vec_t *q);			// for all Nj
  void pN_equals_ANC_times_qC (vec_t *p, vec_t *q);	// for all Nj
  ...
  void pN_plusequals_s_times_qN (vec_t *p, vec_t s, vec_t *q); // for all Nj
...
# endif
  ...
  if (nub > 0) {
    for (i=0; i<nub; i++) memcpy (_L+i*nskip,AROW(i),(i+1)*sizeof(vec_t));
    dFactorLDLT (_L,_d,nub,nskip);
    memcpy (x,b,nub*sizeof(vec_t));
...
vec_t dLCP::Aii (int i)
{
  return AROW(i)[i];
}


vec_t dLCP::AiC_times_qC (int i, vec_t *q)
{
  vec_t sum = 0;
...
vec_t dLCP::AiN_times_qN (int i, vec_t *q)
{
  vec_t sum = 0;
...
void dLCP::pN_equals_ANC_times_qC (vec_t *p, vec_t *q)
{
  vec_t sum;
...
void dLCP::pN_plusequals_ANi (vec_t *p, int i, int sign)
{
  int k;
...
void dLCP::pC_plusequals_s_times_qC (vec_t *p, vec_t s, vec_t *q)
{
  for (int k=0; k<n; k++) if (C[k]) p[k] += s*q[k];
}


void dLCP::pN_plusequals_s_times_qN (vec_t *p, vec_t s, vec_t *q)
{
  for (int k=0; k<n; k++) if (N[k]) p[k] += s*q[k];
}


void dLCP::solve1 (vec_t *a, int i, int dir, int only_transfer)
{

  ALLOCA (vec_t,AA,n*nskip*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (AA == NULL) {
      dMemoryFlag = d_MEMORY_OUT_OF_MEMORY;
      return;
...
#endif
  ALLOCA (vec_t,dd,n*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (dd == NULL) {
      UNALLOCA(AA);
      dMemoryFlag = d_MEMORY_OUT_OF_MEMORY;
...
#endif
  ALLOCA (vec_t,bb,n*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (bb == NULL) {
      UNALLOCA(AA);
      UNALLOCA(dd);
...
# endif
  ...
  */
  ...
  for (k=nub; k<n; k++) {
    ...
    if (lo[k]==-X_infinity && hi[k]==X_infinity) {
  ...
  if (nub > 0) {
    for (k=0; k<nub; k++) memcpy (L+k*nskip,AROW(k),(k+1)*sizeof(vec_t));
  ...
  for (k=0; k<n; k++) {
    if (k<nub) printf ("C");
    else if (lo[k]==-X_infinity && hi[k]==X_infinity) printf ("c");
    else printf (".");
  }
  printf ("\n");
  */
...
void dLCP::transfer_i_to_C (int i)
{
  ...
  if (nC > 0) {
    ...
    d[nC] = X_recip (AROW(i)[i] - dDot(ell,Dell,nC));
  }
  else {
    d[0] = X_recip (AROW(i)[i]);
  }
...
void dLCP::unpermute()
{
  ...
  int j;
  ALLOCA (vec_t,tmp,n*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (tmp == NULL) {
      dMemoryFlag = d_MEMORY_OUT_OF_MEMORY;
      return;
...
#endif
  memcpy (tmp,x,n*sizeof(vec_t));
  for (j=0; j<n; j++) x[p[j]] = tmp[j];
  memcpy (tmp,w,n*sizeof(vec_t));
...
// must have lo=0, hi=X_infinity, and nub=0.

void dSolveLCPBasic (int n, vec_t *A, vec_t *x, vec_t *b,
		     vec_t *w, int nub, vec_t *lo, vec_t *hi)
{
  dAASSERT (n>0 && A && x && b && w && nub == 0);

  int i,k;
  int nskip = dPAD(n);
  ALLOCA (vec_t,L,n*nskip*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (L == NULL) {
      dMemoryFlag = d_MEMORY_OUT_OF_MEMORY;
      return;
...
#endif
  ALLOCA (vec_t,delta_x,n*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (delta_x == NULL) {
      UNALLOCA(d);
      UNALLOCA(L);
...
#endif
  ALLOCA (vec_t,delta_w,n*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (delta_w == NULL) {
      UNALLOCA(delta_x);
      UNALLOCA(d);
...
#endif
  ALLOCA (vec_t,Dell,n*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (Dell == NULL) {
      UNALLOCA(delta_w);
      UNALLOCA(delta_x);
...
#endif
  ALLOCA (vec_t,ell,n*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (ell == NULL) {
      UNALLOCA(Dell);
      UNALLOCA(delta_w);
...
#endif
  ALLOCA (vec_t*,Arows,n*sizeof(vec_t*));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (Arows == NULL) {
      UNALLOCA(tmp);
      UNALLOCA(ell);
  ...
  for (i=0; i<n; i++) {
	...
	for (k=0; k < lcp.numN(); k++) {
	  if (delta_w[lcp.indexN(k)] < 0) {
	    vec_t s2 = -w[lcp.indexN(k)] / delta_w[lcp.indexN(k)];
	    if (s2 < s) {
	      s = s2;
	      si = lcp.indexN(k);
	      si_in_N = 1;
	...
	for (k=0; k < lcp.numC(); k++) {
	  if (delta_x[lcp.indexC(k)] < 0) {
	    vec_t s2 = -x[lcp.indexC(k)] / delta_x[lcp.indexC(k)];
	    if (s2 < s) {
	      s = s2;
	      si = lcp.indexC(k);
	      si_in_N = 0;
...
void dSolveLCP (int n, vec_t *A, vec_t *x, vec_t *b,
		vec_t *w, int nub, vec_t *lo, vec_t *hi, int *findex)
{
...
# endif
  ...
  for (i=nub; i<n; i++) {
    ...
    if (hit_first_friction_index == 0 && findex && findex[i] >= 0) {
      ...
      for (k=i; k<n; k++) {
	vec_t wfk = delta_w[findex[k]];
	if (wfk == 0) {
	  hi[k] = 0;
	  lo[k] = 0;
	}
	else {
	  hi[k] = X_fabs (hi[k] * wfk);
	  lo[k] = -hi[k];
	}
      }
	...
	if (dir > 0) {
	  if (hi[i] < X_infinity) {
	    vec_t s2 = (hi[i]-x[i])/dirf;		// step to x(i)=hi(i)
	    if (s2 < s) {
	      s = s2;
	      cmd = 3;
	    }
	  ...
	  if (lo[i] > -X_infinity) {
	    vec_t s2 = (lo[i]-x[i])/dirf;		// step to x(i)=lo(i)
	    if (s2 < s) {
	      s = s2;
	      cmd = 2;
	    }
	...
	for (k=nub; k < lcp->numC(); k++) {
	  if (delta_x[lcp->indexC(k)] < 0 && lo[lcp->indexC(k)] > -X_infinity) {
	    vec_t s2 = (lo[lcp->indexC(k)]-x[lcp->indexC(k)]) /
	  ...
	  if (delta_x[lcp->indexC(k)] > 0 && hi[lcp->indexC(k)] < X_infinity) {
	    vec_t s2 = (hi[lcp->indexC(k)]-x[lcp->indexC(k)]) /
	      delta_x[lcp->indexC(k)];
	    if (s2 < s) {
	      s = s2;
	      cmd = 6;
...
extern "C" void dTestSolveLCP()
{
  int n = 100;
  int i,nskip = dPAD(n);
  const vec_t tol = REAL(1e-9);
  printf ("dTestSolveLCP()\n");

  ALLOCA (vec_t,A,n*nskip*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (A == NULL) {
      dMemoryFlag = d_MEMORY_OUT_OF_MEMORY;
      return;
...
#endif
  ALLOCA (vec_t,lo,n*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (lo == NULL) {
      UNALLOCA (w);
      UNALLOCA (b);
...
#endif
  ALLOCA (vec_t,hi,n*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (hi == NULL) {
      UNALLOCA (lo);
      UNALLOCA (w);
  ...
  ALLOCA (vec_t,A2,n*nskip*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (A2 == NULL) {
      UNALLOCA (hi);
      UNALLOCA (lo);
...
#endif
  ALLOCA (vec_t,b2,n*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (b2 == NULL) {
      UNALLOCA (A2);
      UNALLOCA (hi);
...
#endif
  ALLOCA (vec_t,lo2,n*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (lo2 == NULL) {
      UNALLOCA (b2);
      UNALLOCA (A2);
...
#endif
  ALLOCA (vec_t,hi2,n*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (hi2 == NULL) {
      UNALLOCA (lo2);
      UNALLOCA (b2);
...
#endif
  ALLOCA (vec_t,tmp1,n*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (tmp1 == NULL) {
      UNALLOCA (hi2);
      UNALLOCA (lo2);
...
#endif
  ALLOCA (vec_t,tmp2,n*sizeof(vec_t));
#ifdef dUSE_MALLOC_FOR_ALLOCA
    if (tmp2 == NULL) {
      UNALLOCA (tmp1);
      UNALLOCA (hi2);
  ...
  for (int count=0; count < 1000; count++) {
    ...
    // make limits
    for (i=0; i<nub; i++) lo[i] = -X_infinity;
    for (i=0; i<nub; i++) hi[i] = X_infinity;
    ...
    */
    ...
    memcpy (A2,A,n*nskip*sizeof(vec_t));
    dClearUpperTriangle (A2,n);
    memcpy (b2,b,n*sizeof(vec_t));
    memcpy (lo2,lo,n*sizeof(vec_t));
    memcpy (hi2,hi,n*sizeof(vec_t));
    dSetZero (x,n);
    dSetZero (w,n);
    ...
    vec_t diff = dMaxDifference (tmp1,tmp2,n,1);
