...
static int findLevel (vec_t bounds[6])
{
  if (bounds[0] <= -X_infinity || bounds[1] >= X_infinity ||
      bounds[2] <= -X_infinity || bounds[3] >= X_infinity ||
      bounds[4] <= -X_infinity || bounds[5] >= X_infinity) {
    return MAXINT;
  }
...
void dxHashSpace::collide (void *data, dNearCallback *callback)
{
  ...
  for (geom = first; geom; geom=geom->next) {
    ...
    if (level <= global_maxlevel) {
      ...
      // cellsize = 2^level
      vec_t cellsize = (vec_t) ldexp (1.0,level);
