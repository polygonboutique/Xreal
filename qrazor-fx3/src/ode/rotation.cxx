...
/*
void dQFromAxisAndAngle (dQuaternion q, vec_t ax, vec_t ay, vec_t az,
			 vec_t angle)
{
  dAASSERT (q);
...
void	dRfromQ(dMatrix3 R, const quaternion_c &q)
{
  ...
  _R(2,2) = 1 - qq1 - qq2;
#else
	vec_t qq1 = 2*q._q[0]*q._q[0];
	vec_t qq2 = 2*q._q[1]*q._q[1];
	vec_t qq3 = 2*q._q[2]*q._q[2];
	
	_R(0,0) = 1 - qq2 - qq3;
	_R(0,1) = 2*(q._q[0]*q._q[1] - q._q[3]*q._q[2]);	
	_R(0,2) = 2*(q._q[0]*q._q[2] + q._q[3]*q._q[1]);
	
	_R(1,0) = 2*(q._q[0]*q._q[1] + q._q[3]*q._q[2]);
	_R(1,1) = 1 - qq1 - qq3;
	_R(1,2) = 2*(q._q[1]*q._q[2] - q._q[3]*q._q[0]);
	
	_R(2,0) = 2*(q._q[0]*q._q[2] - q._q[3]*q._q[1]);
	_R(2,1) = 2*(q._q[1]*q._q[2] + q._q[3]*q._q[0]);
	_R(2,2) = 1 - qq1 - qq2;
#endif
}
*/
...
void dQfromR (dQuaternion q, const dMatrix3 R)
{
  ...
  if (tr >= 0) {
    ...
    case_0:
    s = X_sqrt((_R(0,0) - (_R(1,1) + _R(2,2))) + 1);
    q[1] = REAL(0.5) * s;
    s = REAL(0.5) * X_recip(s);
    q[2] = (_R(0,1) + _R(1,0)) * s;
    q[3] = (_R(2,0) + _R(0,2)) * s;
    ...
    case_1:
    s = X_sqrt((_R(1,1) - (_R(2,2) + _R(0,0))) + 1);
    q[2] = REAL(0.5) * s;
    s = REAL(0.5) * X_recip(s);
    q[3] = (_R(1,2) + _R(2,1)) * s;
    q[1] = (_R(0,1) + _R(1,0)) * s;
    ...
    case_2:
    s = X_sqrt((_R(2,2) - (_R(0,0) + _R(1,1))) + 1);
    q[3] = REAL(0.5) * s;
    s = REAL(0.5) * X_recip(s);
    q[1] = (_R(2,0) + _R(0,2)) * s;
    q[2] = (_R(1,2) + _R(2,1)) * s;
