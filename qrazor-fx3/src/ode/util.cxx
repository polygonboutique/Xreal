...
void dInternalHandleAutoDisabling (dxWorld *world, vec_t stepsize)
{
	dxBody *bb;
	for (bb=world->firstbody; bb; bb=(dxBody*)bb->next) {
		...
		vec_t lspeed2 = dDOT(bb->lvel,bb->lvel);
		if (lspeed2 > bb->adis.linear_threshold) {
			idle = 0;		// moving fast - not idle
		}
		else {
			vec_t aspeed = dDOT(bb->avel,bb->avel);
...
static inline vec_t	sinc(vec_t x)
{
...
void dxStepBody (dxBody *b, vec_t h)
{
  int j;
  ...
  if (b->flags & dxBodyFlagFiniteRotation) {
    ...
    quaternion_c q;	// quaternion for finite rotation
    ...
    if (b->flags & dxBodyFlagFiniteRotationAxis)
    {
      ...
      vec_t k = dDOT (b->finite_rot_axis,b->avel);
      ...
      h *= REAL(0.5);
      vec_t theta = k * h;
      vec_t s = sinc(theta) * h;
      
      q.set(frv[0] * s, frv[1] * s, frv[2] * s, X_cos(theta));
    }
    else
    {
      // make a rotation quaternion q that corresponds to w * h
      vec_t wlen = X_sqrt (b->avel[0]*b->avel[0] + b->avel[1]*b->avel[1] +
			  b->avel[2]*b->avel[2]);
      h *= REAL(0.5);
      vec_t theta = wlen * h;
      vec_t s = sinc(theta) * h;
      
      q.set(b->avel[0] * s, b->avel[1] * s, b->avel[2] * s, X_cos(theta));
    }
...
void dxProcessIslands (dxWorld *world, vec_t stepsize, dstepper_fn_t stepper)
{
  dxBody *b,*bb,**body;
  dxJoint *j,**joint;
