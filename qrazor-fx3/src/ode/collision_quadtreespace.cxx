...
public:
	...
	bool Inside(const vec_t* AABB);
	
	Block* GetBlock(const vec_t* AABB);
	Block* GetBlockChild(const vec_t* AABB);
};
...
Block* Block::GetBlockChild(const vec_t* AABB){
	if (Children){
		for (int i = 0; i < SPLITS; i++){
