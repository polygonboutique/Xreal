...
struct dxAutoDisable {
  vec_t linear_threshold;	// linear (squared) velocity treshold
  vec_t angular_threshold;	// angular (squared) velocity treshold
  vec_t idle_time;		// time the body needs to be idle to auto-disable it
...
struct dxQuickStepParameters {
  ...
  vec_t w;			// the SOR over-relaxation parameter
};


// contact generation parameters
struct dxContactParameters {
  vec_t max_vel;		// maximum correcting velocity
  vec_t min_depth;		// thickness of 'surface layer'
};
...
struct dxBody : public dObject {
  ...
  matrix_c invI;		// inverse of mass.I
  vec_t invMass;		// 1 / mass.mass
  dVector3 pos;			// position of POR (point of reference)
  quaternion_c q;		// orientation quaternion
  matrix_c R;			// rotation matrix, always corresponds to q
  ...
  vec_t adis_timeleft;		// time left to be idle
...
struct dxWorld : public dBase {
  ...
  vec_t global_erp;		// global error reduction parameter
  vec_t global_cfm;		// global costraint force mixing parameter
  dxAutoDisable adis;		// auto-disable parameters
