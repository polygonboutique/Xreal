...
class dMatrix {
  ...
  vec_t *data;		// if nonzero, n*m elements allocated on the heap
...
public:
  ...
  dMatrix (int rows, int cols, vec_t *_data, int rowskip, int colskip);
  ~dMatrix();				// destructor

  // data movement
  vec_t & operator () (int i, int j);	// reference an element
  void operator= (const dMatrix &);	// matrix = matrix
  void operator= (vec_t);		// matrix = scalar
  ...
  void makeRandom (vec_t range);
  void print (char *fmt = "%10.4f ", FILE *f=stdout);
  vec_t maxDifference (const dMatrix &);
};
