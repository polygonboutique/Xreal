...
int dxGeom::AABBTest (dxGeom *o, vec_t aabb[6])
{
  return 1;
}
...
void dGeomSetBody (dxGeom *g, dxBody *b)
{
  ...
  if (b) {
    ...
    if (g->body) {
      ...
      *g->R = g->body->R;	//memcpy (g->R, g->body->R, sizeof(matrix_c));
      g->bodyRemove();
    }
...
void dGeomSetPosition (dxGeom *g, vec_t x, vec_t y, vec_t z)
{
  dAASSERT (g);
  dUASSERT (g->gflags & GEOM_PLACEABLE,"geom must be placeable");
  CHECK_NOT_LOCKED (g->parent_space);
...
void dGeomSetRotation (dxGeom *g, const matrix_c &R)
{
	dAASSERT(g);
	dUASSERT(g->gflags & GEOM_PLACEABLE, "geom must be placeable");
	...
	{
		*g->R = R;	//memcpy(g->R, R, sizeof(dMatrix3));
		dGeomMoved(g);
	}
}


void	dGeomSetQuaternion(dxGeom *g, const quaternion_c &quat)
{
	dAASSERT(g);
	dUASSERT(g->gflags & GEOM_PLACEABLE, "geom must be placeable");
	...
	{
		g->R->fromQuaternion(quat);
		dGeomMoved(g);
	}
}


const vec_t* dGeomGetPosition(dxGeom *g)
{
	dAASSERT(g);
	dUASSERT(g->gflags & GEOM_PLACEABLE, "geom must be placeable");
...
/*
const vec_t* dGeomGetRotation(dxGeom *g)
{
	dAASSERT(g);
	dUASSERT(g->gflags & GEOM_PLACEABLE, "geom must be placeable");
...
const quaternion_c&	dGeomGetQuaternion(dxGeom *g)
{
	dAASSERT(g);
	dUASSERT(g->gflags & GEOM_PLACEABLE, "geom must be placeable");
	...
	{
		/*
		const vec_t * body_quat = dBodyGetQuaternion (g->body);
		quat[0] = body_quat[0];
		quat[1] = body_quat[1];
		quat[2] = body_quat[2];
		quat[3] = body_quat[3];
		*/
		...
		return dBodyGetQuaternion(g->body);
	}
	else
	{
		static quaternion_c quat;
		quat.fromMatrix(*g->R);
		return quat;
	}
}
...
void dGeomGetAABB (dxGeom *g, vec_t aabb[6])
{
  dAASSERT (g);
  dAASSERT (aabb);
  g->recomputeAABB();
  memcpy (aabb,g->aabb,6 * sizeof(vec_t));
}
...
struct dxUserGeom : public dxGeom {
  ...
  ~dxUserGeom();
  void computeAABB();
  int AABBTest (dxGeom *o, vec_t aabb[6]);
};
...
int dxUserGeom::AABBTest (dxGeom *o, vec_t aabb[6])
{
  dGeomClass *c = &user_classes[type-dFirstUserClass];
