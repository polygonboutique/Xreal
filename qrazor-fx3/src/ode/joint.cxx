			    ...
			    dVector3 anchor1, dVector3 anchor2)
{
  ...
  info->J1l[s+1] = 1;
  info->J1l[2*s+2] = 1;
  dMULTIPLY0_331(a1, (vec_t*)joint->node[0].body->R, anchor1);
  dCROSSMAT (info->J1a,a1,s,-,+);
  if (joint->node[1].body) {
    info->J2l[0] = -1;
    info->J2l[s+1] = -1;
    info->J2l[2*s+2] = -1;
    dMULTIPLY0_331 (a2, (vec_t*)joint->node[1].body->R, anchor2);
    dCROSSMAT (info->J2a,a2,s,+,-);
  }

  // set right hand side
  vec_t k = info->fps * info->erp;
  if (joint->node[1].body) {
    for (int j=0; j<3; j++) {
...
static void setFixedOrientation(dxJoint *joint, dxJoint::Info2 *info, quaternion_c &qrel, int start_row)
{
  int s = info->rowskip;
  ...
  {
    quaternion_c qq;
    qq.multiply1(joint->node[0].body->q, joint->node[1].body->q);
    qerr.multiply2(qq, qrel);
  }
  else
  {
    qerr.multiply3(joint->node[0].body->q, qrel);
  }
  
  if(((vec_t*)qerr)[3] < 0)
  {
  	qerr.inverse();		// adjust sign of qerr to make theta small
  }
  
  dMULTIPLY0_331((vec_t*)e, (vec_t*)joint->node[0].body->R, (vec_t*)qerr); // @@@ bad SIMD padding!	//FIXME
  vec_t k = info->fps * info->erp;
...
static void	setAnchors(dxJoint *j, vec_t x, vec_t y, vec_t z, dVector3 anchor1, dVector3 anchor2)
{
	if(j->node[0].body)
	{
		vec_t q[4];
		...
		dMULTIPLY1_331(anchor1, (vec_t*)j->node[0].body->R, q);
		...
		if(j->node[1].body)
		{
			...
			dMULTIPLY1_331 (anchor2, (vec_t*)j->node[1].body->R, q);
...
static void	setAxes(dxJoint *j, vec_t x, vec_t y, vec_t z, dVector3 axis1, dVector3 axis2)
{
	if(j->node[0].body)
	{
		vec_t q[4];
		...
		if(axis1)
		{
			dMULTIPLY1_331(axis1, (vec_t*)j->node[0].body->R, q);
			axis1[3] = 0;
		}
		...
		if(axis2)
		{
			if(j->node[1].body)
			{
				dMULTIPLY1_331(axis2, (vec_t*)j->node[1].body->R, q);
			}
			else
			{
				axis2[0] = x;
...
{
  if (j->node[0].body) {
    dMULTIPLY0_331 (result,(vec_t*)j->node[0].body->R,anchor1);
...
{
  if (j->node[1].body) {
    dMULTIPLY0_331 (result,(vec_t*)j->node[1].body->R,anchor2);
...
{
  if (j->node[0].body) {
    dMULTIPLY0_331 (result,(vec_t*)j->node[0].body->R,axis1);
  }
}
...
{
  if (j->node[1].body) {
    dMULTIPLY0_331 (result,(vec_t*)j->node[1].body->R,axis2);
  }
  else {
    result[0] = axis2[0];
    result[1] = axis2[1];
...
static vec_t	getHingeAngleFromRelativeQuat(const quaternion_c &qrel, const dVector3 axis)
{
  ...
  // sint2 = |sin(theta/2)|
	vec_t cost2 = qrel[3];
	vec_t sint2 = X_sqrt(qrel[0]*qrel[0]+qrel[1]*qrel[1]+qrel[2]*qrel[2]);
	vec_t theta = (dDOT(qrel, axis) >= 0) ?	// @@@ padding assumptions
	(2 * X_atan2(sint2,cost2)) :		// if u points in direction of axis
	(2 * X_atan2(sint2,-cost2));		// if u points in opposite direction
...
static vec_t	getHingeAngle(dxBody *body1, dxBody *body2, dVector3 axis, const quaternion_c &q_initial)
{
	...
	if(body2)
	{
		...
		qq.multiply1(body1->q, body2->q);
		qrel.multiply2(qq, q_initial);
	}
	else
	{
		// pretend body2->q is the identity
		qrel.multiply3(body1->q, q_initial);
	}
...
void dxJointLimitMotor::set (int num, vec_t value)
{
  switch (num) {
  case dParamLoStop:
...
vec_t dxJointLimitMotor::get (int num)
{
  switch (num) {
...
int dxJointLimitMotor::testRotationalLimit (vec_t angle)
{
  if (angle <= lostop) {
    limit = 1;
				 ...
				 dVector3 ax1, int rotational)
{
  ...
  int powered = fmax > 0;
  if (powered || limit) {
    vec_t *J1 = rotational ? info->J1a : info->J1l;
    vec_t *J2 = rotational ? info->J2a : info->J2l;
    ...
    if (limit) {
      vec_t k = info->fps * stop_erp;
      info->c[row] = -k * limit_err;
      info->cfm[row] = stop_cfm;

      if (lostop == histop) {
	// limited low and high simultaneously
	info->lo[row] = -X_infinity;
	info->hi[row] = X_infinity;
      }
      else {
	if (limit == 1) {
	  // low limit
...
static void hingeGetInfo1 (dxJointHinge *j, dxJoint::Info1 *info)
{
  ...
  if ((j->limot.lostop >= -M_PI || j->limot.histop <= M_PI) &&
    ...
    vec_t angle = getHingeAngle (j->node[0].body,j->node[1].body,j->axis1,
				 j->qrel);
...
static void hingeGetInfo2 (dxJointHinge *joint, dxJoint::Info2 *info)
{
  ...
  dMULTIPLY0_331 (ax1,(vec_t*)joint->node[0].body->R,joint->axis1);
  dPlaneSpace (ax1,p,q);
  ...
  dVector3 ax2,b;
  if (joint->node[1].body) {
    dMULTIPLY0_331 (ax2,(vec_t*)joint->node[1].body->R,joint->axis2);
  }
  else {
    ax2[0] = joint->axis2[0];
    ax2[1] = joint->axis2[1];
...
static void	hingeComputeInitialRelativeRotation(dxJointHinge *joint)
{
	if(joint->node[0].body)
	{
		if(joint->node[1].body)
		{
			joint->qrel.multiply1(joint->node[0].body->q, joint->node[1].body->q);
		}
		else
		{
			// set joint->qrel to the transpose of the first body q
			joint->qrel[3] = joint->node[0].body->q[3];
					   ...
					   vec_t dx, vec_t dy, vec_t dz)
{
  ...
  if (joint->node[0].body) {
    ...
    q[3] = 0;
    dMULTIPLY1_331 (joint->anchor1,(vec_t*)joint->node[0].body->R,q);
    ...
    if (joint->node[1].body) {
      ...
      q[3] = 0;
      dMULTIPLY1_331 (joint->anchor2,(vec_t*)joint->node[1].body->R,q);
    }
    else {
...
extern "C" vec_t dJointGetHingeParam (dxJointHinge *joint, int parameter)
{
  dUASSERT(joint,"bad joint argument");
  dUASSERT(joint->vtable == &__dhinge_vtable,"joint is not a hinge");
...
extern "C" vec_t dJointGetHingeAngle (dxJointHinge *joint)
{
  dAASSERT(joint);
  dUASSERT(joint->vtable == &__dhinge_vtable,"joint is not a hinge");
  if (joint->node[0].body) {
    vec_t ang = getHingeAngle (joint->node[0].body,joint->node[1].body,joint->axis1,
			  joint->qrel);
	if (joint->flags & dJOINT_REVERSE)
	   return -ang;
	else
...
extern "C" vec_t dJointGetHingeAngleRate (dxJointHinge *joint)
{
  dAASSERT(joint);
  dUASSERT(joint->vtable == &__dhinge_vtable,"joint is not a Hinge");
  if (joint->node[0].body) {
    dVector3 axis;
    dMULTIPLY0_331 (axis,(vec_t*)joint->node[0].body->R,joint->axis1);
    vec_t rate = dDOT(axis,joint->node[0].body->avel);
...
extern "C" void dJointAddHingeTorque (dxJointHinge *joint, vec_t torque)
{
  dVector3 axis;
  dAASSERT(joint);
  dUASSERT(joint->vtable == &__dhinge_vtable,"joint is not a Hinge");
...
extern "C" vec_t dJointGetSliderPosition (dxJointSlider *joint)
{
  dUASSERT(joint,"bad joint argument");
  ...
  if (joint->node[1].body) {
    ...
    dMULTIPLY0_331 (q,(vec_t*)joint->node[1].body->R,joint->offset);
...
extern "C" vec_t dJointGetSliderPositionRate (dxJointSlider *joint)
{
  dUASSERT(joint,"bad joint argument");
  dUASSERT(joint->vtable == &__dslider_vtable,"joint is not a slider");
...
static void sliderGetInfo1 (dxJointSlider *j, dxJoint::Info1 *info)
{
  ...
  // see if we're at a joint limit.
  j->limot.limit = 0;
  if ((j->limot.lostop > -X_infinity || j->limot.histop < X_infinity) &&
      j->limot.lostop <= j->limot.histop) {
    // measure joint position
    vec_t pos = dJointGetSliderPosition (j);
...
extern "C" void	dJointSetSliderAxis(dxJointSlider *joint, vec_t x, vec_t y, vec_t z)
{
	dUASSERT(joint, "bad joint argument");
	dUASSERT(joint->vtable == &__dslider_vtable, "joint is not a slider");
	...
	if(joint->node[1].body)
	{
		...
		dMULTIPLY1_331(joint->offset, (vec_t*)joint->node[1].body->R, c);
	}
	else
	{
		...
		*/
		...
		joint->qrel = joint->node[0].body->q;
		joint->qrel.inverse();
		
		joint->offset[0] = joint->node[0].body->pos[0];
		joint->offset[1] = joint->node[0].body->pos[1];
		joint->offset[2] = joint->node[0].body->pos[2];
	}
}


extern "C" void	dJointSetSliderAxisDelta(dxJointSlider *joint, vec_t x, vec_t y, vec_t z, vec_t dx, vec_t dy, vec_t dz)
{
	dUASSERT(joint, "bad joint argument");
	dUASSERT(joint->vtable == &__dslider_vtable, "joint is not a slider");
...
extern "C" vec_t dJointGetSliderParam (dxJointSlider *joint, int parameter)
{
  dUASSERT(joint,"bad joint argument");
  dUASSERT(joint->vtable == &__dslider_vtable,"joint is not a slider");
  return joint->limot.get (parameter);
...
extern "C" void dJointAddSliderForce (dxJointSlider *joint, vec_t force)
{
  dVector3 axis;
  dUASSERT(joint,"bad joint argument");
  dUASSERT(joint->vtable == &__dslider_vtable,"joint is not a slider");
...
static void contactGetInfo1 (dxJointContact *j, dxJoint::Info1 *info)
{
  ...
  if (j->contact.surface.mode & dContactMu2) {
    ...
    if (j->contact.surface.mu  == X_infinity) nub ++;
    if (j->contact.surface.mu2 == X_infinity) nub ++;
  }
  else {
    if (j->contact.surface.mu > 0) m += 2;
    if (j->contact.surface.mu == X_infinity) nub += 2;
  }
...
static void contactGetInfo2 (dxJointContact *j, dxJoint::Info2 *info)
{
  ...
  if (j->contact.surface.mode & dContactBounce) {
    ...
    vec_t outgoing = dDOT(info->J1l,j->node[0].body->lvel) +
	...
	(-outgoing) > j->contact.surface.bounce_vel) {
      vec_t newc = - j->contact.surface.bounce * outgoing;
...
static vec_t measureHinge2Angle (dxJointHinge2 *joint)
{
  dVector3 a1,a2;
  dMULTIPLY0_331 (a1,(vec_t*)joint->node[1].body->R,joint->axis2);
  dMULTIPLY1_331 (a2,(vec_t*)joint->node[0].body->R,a1);
  vec_t x = dDOT(joint->v1,a2);
  vec_t y = dDOT(joint->v2,a2);
  return -X_atan2 (y,x);
}
...
static void hinge2GetInfo1 (dxJointHinge2 *j, dxJoint::Info1 *info)
{
  ...
  if ((j->limot1.lostop >= -M_PI || j->limot1.histop <= M_PI) &&
    ...
    vec_t angle = measureHinge2Angle (j);
...
#define HINGE2_GET_AXIS_INFO(axis,sin_angle,cos_angle) \
  ...
  dMULTIPLY0_331 (ax2,(vec_t*)joint->node[1].body->R,joint->axis2); \
  dCROSS (axis,=,ax1,ax2); \
  sin_angle = X_sqrt (axis[0]*axis[0] + axis[1]*axis[1] + axis[2]*axis[2]); \
...
static void makeHinge2V1andV2 (dxJointHinge2 *joint)
{
  if (joint->node[0].body) {
    ...
    vec_t k = dDOT(ax1,ax2);
    for (int i=0; i<3; i++) ax2[i] -= k*ax1[i];
    dNormalize3 (ax2);

    // make v1 = modified axis2, v2 = axis1 x (modified axis2)
    dCROSS (v,=,ax1,ax2);
    dMULTIPLY1_331 (joint->v1,(vec_t*)joint->node[0].body->R,ax2);
    dMULTIPLY1_331 (joint->v2,(vec_t*)joint->node[0].body->R,v);
  }
}
				      ...
				      vec_t x, vec_t y, vec_t z)
{
  ...
  if (joint->node[0].body) {
    ...
    q[3] = 0;
    dNormalize3 (q);
    dMULTIPLY1_331 (joint->axis1,(vec_t*)joint->node[0].body->R,q);
    joint->axis1[3] = 0;
				      ...
				      vec_t x, vec_t y, vec_t z)
{
  ...
  if (joint->node[1].body) {
    ...
    q[3] = 0;
    dNormalize3 (q);
    dMULTIPLY1_331 (joint->axis2,(vec_t*)joint->node[1].body->R,q);
    joint->axis1[3] = 0;
...
extern "C" void dJointGetHinge2Axis1 (dxJointHinge2 *joint, dVector3 result)
{
  ...
  dUASSERT(joint->vtable == &__dhinge2_vtable,"joint is not a hinge2");
  if (joint->node[0].body) {
    dMULTIPLY0_331 (result,(vec_t*)joint->node[0].body->R,joint->axis1);
  }
}
...
extern "C" void dJointGetHinge2Axis2 (dxJointHinge2 *joint, dVector3 result)
{
  ...
  dUASSERT(joint->vtable == &__dhinge2_vtable,"joint is not a hinge2");
  if (joint->node[1].body) {
    dMULTIPLY0_331 (result,(vec_t*)joint->node[1].body->R,joint->axis2);
  }
}


extern "C" vec_t dJointGetHinge2Param (dxJointHinge2 *joint, int parameter)
{
  dUASSERT(joint,"bad joint argument");
  dUASSERT(joint->vtable == &__dhinge2_vtable,"joint is not a hinge2");
  if ((parameter & 0xff00) == 0x100) {
...
extern "C" vec_t dJointGetHinge2Angle1 (dxJointHinge2 *joint)
{
  dUASSERT(joint,"bad joint argument");
  dUASSERT(joint->vtable == &__dhinge2_vtable,"joint is not a hinge2");
...
extern "C" vec_t dJointGetHinge2Angle1Rate (dxJointHinge2 *joint)
{
  dUASSERT(joint,"bad joint argument");
  dUASSERT(joint->vtable == &__dhinge2_vtable,"joint is not a hinge2");
  if (joint->node[0].body) {
...
extern "C" vec_t dJointGetHinge2Angle2Rate (dxJointHinge2 *joint)
{
  dUASSERT(joint,"bad joint argument");
  dUASSERT(joint->vtable == &__dhinge2_vtable,"joint is not a hinge2");
  if (joint->node[0].body && joint->node[1].body) {
    dVector3 axis;
    dMULTIPLY0_331 (axis,(vec_t*)joint->node[1].body->R,joint->axis2);
    vec_t rate = dDOT(axis,joint->node[0].body->avel);
    if (joint->node[1].body) rate -= dDOT(axis,joint->node[1].body->avel);
    return rate;
  }
...
extern "C" void dJointAddHinge2Torques (dxJointHinge2 *joint, vec_t torque1, vec_t torque2)
{
  ...
  if (joint->node[0].body && joint->node[1].body) {
    dMULTIPLY0_331 (axis1,(vec_t*)joint->node[0].body->R,joint->axis1);
    dMULTIPLY0_331 (axis2,(vec_t*)joint->node[1].body->R,joint->axis2);
...
static vec_t	getUniversalAngle1(dxJointUniversal *joint)
{
	if(joint->node[0].body)
	{
		...
		R.from2Axes(ax1[0], ax1[1], ax1[2], ax2[0], ax2[1], ax2[2]);
		qcross.fromMatrix(R);
		...
		qq.multiply1(joint->node[0].body->q, qcross);
		qrel.multiply2(qq, joint->qrel1);
...
static vec_t	getUniversalAngle2(dxJointUniversal *joint)
{
	if(joint->node[0].body)
	{
		// length 1 joint axis in global coordinates, from each body
		...
		// other than the given axis.  What I have here works,
		// although it's probably much slower than need be.
		R.from2Axes(ax2[0], ax2[1], ax2[2], ax1[0], ax1[1], ax1[2]);
		qcross.fromMatrix(R);
		    
		if(joint->node[1].body)
		{
			qq.multiply1(joint->node[1].body->q, qcross);
			qrel.multiply2(qq, joint->qrel2);
		}
		else
		{
			// pretend joint->node[1].body->q is the identity
			qrel.multiply2(qcross, joint->qrel2);
		}
...
static void universalComputeInitialRelativeRotations	(dxJointUniversal *joint)
{
	if(joint->node[0].body)
	{
		...
		R.from2Axes(ax1[0], ax1[1], ax1[2], ax2[0], ax2[1], ax2[2]);
		qcross.fromMatrix(R);
		joint->qrel1.multiply1(joint->node[0].body->q, qcross);
		...
		if(joint->node[1].body)
		{
			joint->qrel2.multiply1(joint->node[1].body->q, qcross);
		}
		else
		{
...
extern "C" vec_t dJointGetUniversalParam (dxJointUniversal *joint, int parameter)
{
  dUASSERT(joint,"bad joint argument");
  dUASSERT(joint->vtable == &__duniversal_vtable,"joint is not a universal");
  if ((parameter & 0xff00) == 0x100) {
...
extern "C" vec_t dJointGetUniversalAngle1 (dxJointUniversal *joint)
{
  dUASSERT(joint,"bad joint argument");
  dUASSERT(joint->vtable == &__duniversal_vtable,"joint is not a universal");
  if (joint->flags & dJOINT_REVERSE)
...
extern "C" vec_t dJointGetUniversalAngle2 (dxJointUniversal *joint)
{
  dUASSERT(joint,"bad joint argument");
  dUASSERT(joint->vtable == &__duniversal_vtable,"joint is not a universal");
  if (joint->flags & dJOINT_REVERSE)
...
extern "C" vec_t dJointGetUniversalAngle1Rate (dxJointUniversal *joint)
{
  dUASSERT(joint,"bad joint argument");
  dUASSERT(joint->vtable == &__duniversal_vtable,"joint is not a universal");
...
extern "C" vec_t dJointGetUniversalAngle2Rate (dxJointUniversal *joint)
{
  dUASSERT(joint,"bad joint argument");
  dUASSERT(joint->vtable == &__duniversal_vtable,"joint is not a universal");
...
extern "C" void dJointAddUniversalTorques (dxJointUniversal *joint, vec_t torque1, vec_t torque2)
{
  dVector3 axis1, axis2;
  dAASSERT(joint);
  dUASSERT(joint->vtable == &__duniversal_vtable,"joint is not a universal");
...
static void amotorComputeGlobalAxes (dxJointAMotor *joint, dVector3 ax[3])
{
  if (joint->mode == dAMotorEuler) {
    ...
    dMULTIPLY0_331 (ax[0],(vec_t*)joint->node[0].body->R,joint->axis[0]);
    if (joint->node[1].body) {
      dMULTIPLY0_331 (ax[2],(vec_t*)joint->node[1].body->R,joint->axis[2]);
    }
    else {
    ...
    for (int i=0; i < joint->num; i++) {
      if (joint->rel[i] == 1) {
	// relative to b1
	dMULTIPLY0_331 (ax[i],(vec_t*)joint->node[0].body->R,joint->axis[i]);
      }
      else if (joint->rel[i] == 2) {
	// relative to b2
	if (joint->node[1].body) {   // jds: don't assert, just ignore
	        dMULTIPLY0_331 (ax[i],(vec_t*)joint->node[1].body->R,joint->axis[i]);
        }
      }
      else {
	// global - just copy it
...
static void amotorComputeEulerAngles (dxJointAMotor *joint, dVector3 ax[3])
{
  ...
  dVector3 ref1,ref2;
  dMULTIPLY0_331 (ref1,(vec_t*)joint->node[0].body->R,joint->reference1);
  if (joint->node[1].body) {
    dMULTIPLY0_331 (ref2,(vec_t*)joint->node[1].body->R,joint->reference2);
  }
  else {
  ...
  dVector3 q;
  dCROSS (q,=,ax[0],ref1);
  joint->angle[0] = -X_atan2 (dDOT(ax[2],q),dDOT(ax[2],ref1));

  // get q perpendicular to both ax[0] and ax[1], get second euler angle
  dCROSS (q,=,ax[0],ax[1]);
  joint->angle[1] = -X_atan2 (dDOT(ax[2],ax[0]),dDOT(ax[2],q));

  // get q perpendicular to both ax[1] and ax[2], get third euler angle
  dCROSS (q,=,ax[1],ax[2]);
  joint->angle[2] = -X_atan2 (dDOT(ref2,ax[1]), dDOT(ref2,q));
}
...
static void amotorSetEulerReferenceVectors (dxJointAMotor *j)
{
  if (j->node[0].body && j->node[1].body) {
    ...
    dMULTIPLY0_331 (r,(vec_t*)j->node[1].body->R,j->axis[2]);
    dMULTIPLY1_331 (j->reference1,(vec_t*)j->node[0].body->R,r);
    dMULTIPLY0_331 (r,(vec_t*)j->node[0].body->R,j->axis[0]);
    dMULTIPLY1_331 (j->reference2,(vec_t*)j->node[1].body->R,r);
  }
				     ...
				     vec_t x, vec_t y, vec_t z)
{
  ...
  if (rel > 0) {
    if (rel==1) {
      dMULTIPLY1_331 (joint->axis[anum],(vec_t*)joint->node[0].body->R,r);
    }
    else {
      // don't assert; handle the case of attachment to a bodiless geom
      if (joint->node[1].body) {   // jds
      dMULTIPLY1_331 (joint->axis[anum],(vec_t*)joint->node[1].body->R,r);
    }
      else {
				     ...
				     dVector3 result)
{
  ...
  if (joint->rel[anum] > 0) {
    if (joint->rel[anum]==1) {
      dMULTIPLY0_331 (result,(vec_t*)joint->node[0].body->R,joint->axis[anum]);
    }
    else {
      if (joint->node[1].body) {   // jds
      dMULTIPLY0_331 (result,(vec_t*)joint->node[1].body->R,joint->axis[anum]);
      }
      else {
...
extern "C" vec_t dJointGetAMotorAngle (dxJointAMotor *joint, int anum)
{
  dAASSERT(joint && anum >= 0 && anum < 3);
  dUASSERT(joint->vtable == &__damotor_vtable,"joint is not an amotor");
  if (anum < 0) anum = 0;
...
extern "C" vec_t dJointGetAMotorAngleRate (dxJointAMotor *joint, int anum)
{
  // @@@
...
extern "C" vec_t dJointGetAMotorParam (dxJointAMotor *joint, int parameter)
{
  dAASSERT(joint);
  dUASSERT(joint->vtable == &__damotor_vtable,"joint is not an amotor");
  int anum = parameter >> 8;
...
extern "C" void dJointAddAMotorTorques (dxJointAMotor *joint, vec_t torque1, vec_t torque2, vec_t torque3)
{
  dVector3 axes[3];
  dAASSERT(joint);
  dUASSERT(joint->vtable == &__damotor_vtable,"joint is not an amotor");
...
# if 0
    ...
    vec_t       *body_z_axis = &joint->node[0].body->R[8];
...
static void fixedGetInfo2 (dxJointFixed *joint, dxJoint::Info2 *info)
{
  ...
  dVector3 ofs;
  dMULTIPLY0_331 (ofs,(vec_t*)joint->node[0].body->R,joint->offset);
  if (joint->node[1].body) {
...
extern "C" void	dJointSetFixed(dxJointFixed *joint)
{
	...
	if(joint->node[0].body)
	{
		if(joint->node[1].body)
		{
			...
			dMULTIPLY1_331(joint->offset, (vec_t*)joint->node[0].body->R, ofs);
		}
		else
		{
			// set joint->qrel to the transpose of the first body's q
