...
struct dxJoint : public dObject {
  ...
  vec_t lambda[6];		// lambda generated by last step
};
...
struct dxJointLimitMotor {
  vec_t vel,fmax;		// powered joint: velocity, max force
  vec_t lostop,histop;		// joint limits, relative to initial position
  vec_t fudge_factor;		// when powering away from joint limits
  vec_t normal_cfm;		// cfm to use when not at a stop
  vec_t stop_erp,stop_cfm;	// erp and cfm for when at joint limit
  vec_t bounce;			// restitution factor
  // variables used between getInfo1() and getInfo2()
  int limit;			// 0=free, 1=at lo limit, 2=at hi limit
  vec_t limit_err;		// if at limit, amount over limit

  void init (dxWorld *);
  void set (int num, vec_t value);
  vec_t get (int num);
  int testRotationalLimit (vec_t angle);
...
struct dxJointHinge : public dxJoint {
  ...
  quaternion_c qrel;		// initial relative rotation body1 -> body2
...
struct dxJointUniversal : public dxJoint {
  ...
  dVector3 axis1;		// axis w.r.t first body
  dVector3 axis2;		// axis w.r.t second body
  quaternion_c qrel1;		// initial relative rotation body1 -> virtual cross piece
  quaternion_c qrel2;		// initial relative rotation virtual cross piece -> body2
...
struct dxJointHinge2 : public dxJoint {
  ...
  vec_t c0,s0;			// cos,sin of desired angle between axis 1,2
  ...
  vec_t susp_erp,susp_cfm;	// suspension parameters (erp,cfm)
};
...
struct dxJointAMotor : public dxJoint {
  ...
  vec_t angle[3];		// user-supplied angles for axes
