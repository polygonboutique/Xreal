...
static g_entity_c*	G_TestEntityPosition(g_entity_c *ent)
{
	...
	if (ent->_r.clipmask)
		...
		mask = MASK_SOLID;
	trace = gi.SV_Trace (ent->_s.origin, ent->_r.bbox, ent->_s.origin, ent, mask);
...
bool	G_RunThink(g_entity_c *ent)
{
	...
	//gi.Com_Printf("SV_RunThink: %s\n", ent->classname);

	thinktime = ent->_nextthink;
...
static int	SV_FlyMove(g_entity_c *ent, float time, int mask)
{
	...
	for (bumpcount=0 ; bumpcount<numbumps ; bumpcount++)
	{
		...
		trace = gi.SV_Trace (ent->_s.origin, ent->_r.bbox, end, ent, mask);
...
retry:
	...
	trace = gi.SV_Trace (start, ent->_r.bbox, end, ent, mask);
...
//retry:
	...
	if(pushed_p > &pushed[MAX_ENTITIES])
		gi.Com_Error(ERR_FATAL, "pushed_p > &pushed[MAX_EDICTS], memory corrupted");
...
static void	G_Physics_Toss(g_entity_c *ent)
{
	...
	ent->_watertype = gi.SV_PointContents (ent->_s.origin);
	...
	if (!wasinwater && isinwater)
		gi.SV_StartSound (old_origin, g_edicts[0], CHAN_AUTO, gi.SV_SoundIndex("misc/h2ohit1.wav"), 1, 1, 0);
		
	else if (wasinwater && !isinwater)
		gi.SV_StartSound (ent->_s.origin, g_edicts[0], CHAN_AUTO, gi.SV_SoundIndex("misc/h2ohit1.wav"), 1, 1, 0);

	// move teamslaves
	for (slave = ent->_teamchain; slave; slave = slave->_teamchain)
	{
		Vector3_Copy (ent->_s.origin, slave->_s.origin);
		gi.SV_LinkEdict (slave);
	}
}
*/
...
static void	G_Physics_Step(g_entity_c *ent)
{
	...
	if (ent->_velocity[2] || ent->_velocity[1] || ent->_velocity[0])
	{
		...
		if (ent->_groundentity)
			if (!wasonground)
				if (hitsound)
					gi.SV_StartSound (NULL, ent, 0, gi.SV_SoundIndex("world/land.wav"), 1, 1, 0);
	}
...
void	G_RunEntity(g_entity_c *ent)
{
	...
	switch(ent->_movetype)
	{
		...
		default:
			gi.Com_Error (ERR_DROP, "G_RunEntity: bad movetype %i", ent->_movetype);
	}
}
...
//	g_ode_world->setContactSurfaceLayer(0.001);
	...
	g_ode_space = new d_simple_space_c();
//	g_ode_space = new d_hash_space_c();
