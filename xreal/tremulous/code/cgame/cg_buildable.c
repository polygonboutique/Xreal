		...
		if((cent->oldBuildableAnim ^ es->legsAnim) & ANIM_TOGGLEBIT)
		{
			if(cg_debugAnim.integer)
				CG_Printf("%d->%d l:%d t:%d %s(%d)\n",
						  ...
						  es->legsAnim, es->torsoAnim, BG_FindHumanNameForBuildable(es->modelindex), es->number);
...
static void CG_BuildableHealthBar(centity_t * cent)
{
	...
	if(!(es->generic1 & B_POWERED_TOGGLEBIT) && BG_FindTeamForBuildable(es->modelindex) == BIT_HUMANS)
	{
...
void CG_Buildable(centity_t * cent)
{
	...
	if(es->pos.trType == TR_STATIONARY)
		CG_PositionAndOrientateBuildable(angles, ent.origin, surfNormal, es->number, mins, maxs, ent.axis, ent.origin);
	...
	switch (cg.predictedPlayerState.weapon)
	{
		...
		case WP_HBUILD:
		case WP_HBUILD2:
			if(BG_FindTeamForBuildable(es->modelindex) == BG_FindTeamForWeapon(cg.predictedPlayerState.weapon))
	...
	if(es->eFlags & EF_FIRING)
	{
		...
		if(cg.time - cent->muzzleFlashTime > MUZZLE_FLASH_TIME || BG_FindProjTypeForBuildable(es->modelindex) == WP_TESLAGEN)
		{
