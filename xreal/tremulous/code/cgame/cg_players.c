...
static void CG_PlayerUpgrades(centity_t * cent, refEntity_t * torso)
{
	...
	if(es->eFlags & EF_BLOBLOCKED)
	{
		...
		if(size > 0.0f)
			CG_ImpactMark(cgs.media.creepShader, origin, up, 0.0f, 1.0f, 1.0f, 1.0f, 1.0f, qfalse, size, qtrue);
	}
}
...
static qboolean CG_PlayerShadow(centity_t * cent, float *shadowPlane, pClass_t class)
{
	...
	*shadowPlane = 0;
	...
	CG_ImpactMark(cgs.media.shadowMarkShader, trace.endpos, trace.plane.normal,
				  cent->pe.legs.yawAngle, 0.0f, 0.0f, 0.0f, alpha, qfalse, 24.0f * BG_FindShadowScaleForClass(class), qtrue);
...
static void CG_PlayerSplash(centity_t * cent, pClass_t class)
{
	...
	CG_ImpactMark(cgs.media.wakeMarkShader, trace.endpos, trace.plane.normal,
				  cent->pe.legs.yawAngle, 1.0f, 1.0f, 1.0f, 1.0f, qfalse, 32.0f * BG_FindShadowScaleForClass(class), qtrue);
}
...
qboolean CG_AtHighestClass(void)
{
	...
	for(i = PCL_NONE + 1; i < PCL_NUM_CLASSES; i++)
	{
		if(BG_ClassCanEvolveFromTo(cg.predictedPlayerState.stats[STAT_PCLASS], i,
								   ALIEN_MAX_KILLS, 0) >= 0 && BG_FindStagesForClass(i, cgs.alienStage) && BG_ClassIsAllowed(i))
		{
