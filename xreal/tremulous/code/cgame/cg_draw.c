			...
			else
			{
				...
				else if(style == ITEM_TEXTSTYLE_NEON)
				{
					...
					CG_Text_PaintChar(x - 3, y - yadj - 3,
									  glyph->imageWidth + 6,
									  glyph->imageHeight + 6, useScale, glyph->s, glyph->t, glyph->s2, glyph->t2, glyph->glyph);
...
static void CG_DrawUsableBuildable(rectDef_t * rect, qhandle_t shader, vec4_t color)
{
	   ...
	   cg.predictedPlayerState.stats[STAT_PTEAM] == BG_FindTeamForBuildable(es->modelindex))
	{
		...
		if((es->modelindex == BA_H_REACTOR || es->modelindex == BA_H_REPEATER) &&
		   (!BG_FindUsesEnergyForWeapon(cg.snap->ps.weapon) || BG_FindInfinteAmmoForWeapon(cg.snap->ps.weapon)))
			return;
...
static void CG_DrawBuildablesProgress(rectDef_t * rect, vec4_t color, float scale, int align, int textStyle, int special)
{
	CG_DrawProgressBar(rect, color, scale, align, textStyle, special, cg.buildablesFraction);
}

static void CG_DrawBuildablesProgressLabel(rectDef_t * rect, float text_x, float text_y, vec4_t color, float scale, int align)
{
	CG_DrawProgressLabel(rect, text_x, text_y, color, scale, align, "Buildable Models", cg.buildablesFraction);
}

static void CG_DrawCharModelProgress(rectDef_t * rect, vec4_t color, float scale, int align, int textStyle, int special)
{
	CG_DrawProgressBar(rect, color, scale, align, textStyle, special, cg.charModelFraction);
}

static void CG_DrawCharModelProgressLabel(rectDef_t * rect, float text_x, float text_y, vec4_t color, float scale, int align)
{
	CG_DrawProgressLabel(rect, text_x, text_y, color, scale, align, "Character Models", cg.charModelFraction);
}

static void CG_DrawOverallProgress(rectDef_t * rect, vec4_t color, float scale, int align, int textStyle, int special)
{
	float           total;
...
static void CG_DrawTextBlock(rectDef_t * rect, float text_x, float text_y, vec4_t color,
							 float scale, int align, int textStyle, const char *text, menuDef_t * parent, itemDef_t * textItem)
{
	float           x, y, w, h;
...
static void CG_DrawCrosshair(void)
{
	...
	if((cg.snap->ps.persistant[PERS_TEAM] == TEAM_SPECTATOR) ||
	   (cg.snap->ps.stats[STAT_STATE] & SS_INFESTING) || (cg.snap->ps.stats[STAT_STATE] & SS_HOVELING))
		return;
...
static void CG_Draw2D(void)
{
	...
	if(!(cg.snap->ps.stats[STAT_STATE] & SS_INFESTING) &&
	   !(cg.snap->ps.stats[STAT_STATE] & SS_HOVELING) && menu && (cg.snap->ps.stats[STAT_HEALTH] > 0))
	{
		if(cg_drawStatus.integer)
