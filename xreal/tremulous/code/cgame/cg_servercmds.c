...
void CG_SetConfigValues(void)
{
	...
	sscanf(CG_ConfigString(CS_BUILDPOINTS),
		   ...
		   &cgs.alienBuildPointsTotal, &cgs.humanBuildPoints, &cgs.humanBuildPointsTotal, &cgs.humanBuildPointsPowered);
...
static void CG_SetUIVars(void)
{
	...
	*carriageCvar = 0;
	...
	{
		if(BG_InventoryContainsWeapon(i, cg.snap->ps.stats) && BG_FindPurchasableForWeapon(i))
			strcat(carriageCvar, va("W%d ", i));
	}
	for(i = UP_NONE + 1; i < UP_NUM_UPGRADES; i++)
	{
		if(BG_InventoryContainsUpgrade(i, cg.snap->ps.stats) && BG_FindPurchasableForUpgrade(i))
...
void CG_Menu(int menu)
{
	...
	switch (menu)
	{
		...
		case MN_H_REACTOR:
			if(!cg_disableWarningDialogs.integer)
			{
				trap_Cvar_Set("ui_dialog", "There can only be one Reactor. Destroy the existing one if you " "wish to move it.");
				trap_SendConsoleCommand("menu tremulous_human_dialog\n");
			}
			else
		...
		case MN_H_NODCC:
			if(!cg_disableWarningDialogs.integer)
			{
				trap_Cvar_Set("ui_dialog", "There is no Defense Computer. A Defense Computer is needed to build " "this.");
				trap_SendConsoleCommand("menu tremulous_human_dialog\n");
			}
			else
		...
		case MN_H_NOFUNDS:
			if(!cg_disableWarningDialogs.integer)
			{
				trap_Cvar_Set("ui_dialog", "Insufficient funds. You do not have enough credits to perform this " "action.");
				trap_SendConsoleCommand("menu tremulous_human_dialog\n");
			}
			else
		...
		case MN_A_OVERMIND:
			if(!cg_disableWarningDialogs.integer)
			{
				trap_Cvar_Set("ui_dialog", "There can only be one Overmind. Destroy the existing one if you " "wish to move it.");
				trap_SendConsoleCommand("menu tremulous_alien_dialog\n");
			}
			else
		...
		case MN_A_HOVEL:
			if(!cg_disableWarningDialogs.integer)
			{
				trap_Cvar_Set("ui_dialog", "There can only be one Hovel. Destroy the existing one if you " "wish to move it.");
				trap_SendConsoleCommand("menu tremulous_alien_dialog\n");
			}
			else
		...
		case MN_A_TOOCLOSE:
			if(!cg_disableWarningDialogs.integer)
			{
				trap_Cvar_Set("ui_dialog", "This location is too close to the enemy to evolve. "
							  "Move away until you are no longer aware of the enemy's " "presence and try again.");
				trap_SendConsoleCommand("menu tremulous_alien_dialog\n");
			}
			else
		...
		case MN_A_NOOVMND_EVOLVE:
			if(!cg_disableWarningDialogs.integer)
			{
				trap_Cvar_Set("ui_dialog", "There is no Overmind. An Overmind must be built to allow " "you to upgrade.");
				trap_SendConsoleCommand("menu tremulous_alien_dialog\n");
			}
			else
				CG_Printf("There is no Overmind\n");
		...
		case MN_A_HOVEL_OCCUPIED:
			if(!cg_disableWarningDialogs.integer)
			{
				trap_Cvar_Set("ui_dialog", "This Hovel is occupied by another builder. Please find or build " "another.");
				trap_SendConsoleCommand("menu tremulous_alien_dialog\n");
			}
			else
		...
		case MN_A_INFEST:
			trap_Cvar_Set("ui_currentClass", va("%d %d", cg.snap->ps.stats[STAT_PCLASS], cg.snap->ps.persistant[PERS_CREDIT]));
