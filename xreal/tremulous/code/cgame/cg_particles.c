...
static particle_t *CG_SpawnNewParticle(baseParticle_t * bp, particleEjector_t * parent)
{
	...
	for(i = 0; i < MAX_PARTICLES; i++)
	{
		...
		if(!p->valid && cg.clientFrame > p->frameWhenInvalidated + 1)
		{
			...
			p->dLightRadius.delay = (int)CG_RandomiseValue((float)bp->dLightRadius.delay, bp->dLightRadius.delayRandFrac);
			...
			switch (bp->velMoveType)
			{
				...
				case PMT_NORMAL:
					if(!ps->normalValid)
					{
						CG_Printf(S_COLOR_RED "ERROR: a particle with velocityType " "normal has no normal\n");
						return NULL;
					}
			...
			if(!ps->lazyRemove)
			{
				while(pe->nextEjectionTime <= cg.time && (pe->count > 0 || pe->totalParticles == PARTICLES_INFINITE))
				{
					...
					pe->nextEjectionTime = cg.time + (int)CG_RandomiseValue(CG_LerpValues(pe->ejectPeriod.initial,
																						  ...
																						  lerpFrac), pe->ejectPeriod.randFrac);
				}
			}
...
static qboolean CG_ParseParticle(baseParticle_t * bp, char **text_p)
{
	...
	while(1)
	{
		...
		else if(!Q_stricmp(token, "shader"))
		{
			if(bp->numModels > 0)
			{
				CG_Printf(S_COLOR_RED "ERROR: 'shader' not allowed in " "conjunction with 'model'\n", token);
				break;
			}
		...
		else if(!Q_stricmp(token, "model"))
		{
			if(bp->numFrames > 0)
			{
				CG_Printf(S_COLOR_RED "ERROR: 'model' not allowed in " "conjunction with 'shader'\n", token);
				break;
			}
...
#define MAX_ACC_RADIUS 1000.0f
	...
	radius = CG_LerpValues(p->radius.initial, p->radius.final, CG_CalculateTimeFrac(p->birthTime, p->lifeTime, p->radius.delay));
...
static void CG_RenderParticle(particle_t * p)
{
	...
	scale = CG_LerpValues(p->radius.initial, p->radius.final, CG_CalculateTimeFrac(p->birthTime, p->lifeTime, p->radius.delay));
	...
	if(bp->numFrames)			//shader based
	{
		...
		else
		{
			...
			VectorMA(bp->initialColor, CG_CalculateTimeFrac(p->birthTime, p->lifeTime, p->colorDelay), colorRange, re.shaderRGBA);
		}
		...
		re.rotation = CG_LerpValues(p->rotation.initial,
									p->rotation.final, CG_CalculateTimeFrac(p->birthTime, p->lifeTime, p->rotation.delay));
	...
	if(bps->thirdPersonOnly && CG_AttachmentCentNum(&ps->attachment) == cg.snap->ps.clientNum && !cg.renderingThirdPerson)
	...
	if(bp->dynamicLight && !(re.renderfx & RF_THIRD_PERSON))
	{
		trap_R_AddLightToScene(p->origin,
							   ...
							   (float)bp->dLightColor[1] / (float)0xFF, (float)bp->dLightColor[2] / (float)0xFF);
	}
