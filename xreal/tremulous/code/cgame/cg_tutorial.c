...
static const char *CG_KeyNameForCommand(const char *command)
{
	...
	for(i = 0; i < numBindings; i++)
	{
		if(!Q_stricmp(command, bindings[i].command))
		{
			...
			{
				Q_strncpyz(buffer, va("\"%s\"", bindings[i].humanName), MAX_STRING_CHARS);
...
static qboolean CG_BuildableInRange(playerState_t * ps)
{
	...
	if(es->eType == ET_BUILDABLE && ps->stats[STAT_PTEAM] == BG_FindTeamForBuildable(es->modelindex))
		return qtrue;
	else
		return qfalse;
}
...
static void CG_AlienBuilderText(char *text, playerState_t * ps)
{
	...
	if(buildable > BA_NONE)
	{
		Q_strcat(text, MAX_TUTORIAL_TEXT,
				 va("Press %s to place the %s\n", CG_KeyNameForCommand("+attack"), BG_FindHumanNameForBuildable(buildable)));

		Q_strcat(text, MAX_TUTORIAL_TEXT,
				 va("Press %s to cancel placing the %s\n",
					CG_KeyNameForCommand("+button5"), BG_FindHumanNameForBuildable(buildable)));
	}
	else
	{
...
static void CG_AlienLevel4Text(char *text, playerState_t * ps)
{
	...
	Q_strcat(text, MAX_TUTORIAL_TEXT, va("Hold down and release %s to charge\n", CG_KeyNameForCommand("+button5")));
}
...
static void CG_HumanText(char *text, playerState_t * ps)
{
	...
	else
	{
		switch (ps->weapon)
		{
			...
			case WP_FLAMER:
				Q_strcat(text, MAX_TUTORIAL_TEXT,
						 va("Press %s to fire the %s\n", CG_KeyNameForCommand("+attack"), BG_FindHumanNameForWeapon(ps->weapon)));
				break;
			...
			case WP_LUCIFER_CANNON:
				...
				Q_strcat(text, MAX_TUTORIAL_TEXT,
						 va("Press %s to fire the %s\n",
							CG_KeyNameForCommand("+button5"), BG_FindHumanNameForWeapon(ps->weapon)));
				break;
	...
	if(upgrade == UP_NONE || (upgrade > UP_NONE && BG_FindUsableForUpgrade(upgrade)))
	{
	...
	{
		Q_strcat(text, MAX_TUTORIAL_TEXT,
				 va("Press %s to use your %s\n", CG_KeyNameForCommand("itemact medkit"), BG_FindHumanNameForUpgrade(UP_MEDKIT)));
	}
...
static void CG_SpectatorText(char *text, playerState_t * ps)
{
	...
	else if(ps->pm_type == PM_SPECTATOR)
	{
		...
		if(CG_PlayerCount() > 0)
		{
			Q_strcat(text, MAX_TUTORIAL_TEXT, va("Press %s to enter spectator follow mode\n", CG_KeyNameForCommand("+button2")));
		}
	}
	else
	{
...
const char     *CG_TutorialText(void)
{
	...
	if(!cg.intermissionStarted && !cg.demoPlayback)
	{
		...
		else if(ps->stats[STAT_HEALTH] > 0)
		{
			...
			if(ps->stats[STAT_PTEAM] == PTE_ALIENS && BG_UpgradeClassAvailable(ps))
			{
