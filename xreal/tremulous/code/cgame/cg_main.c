...
static void CG_RegisterSounds(void)
{
	...
	for(i = 0; i < 4; i++)
		cgs.media.humanBuildableDamage[i] = trap_S_RegisterSound(va("sound/buildables/human/damage%d.wav", i), qfalse);
...
static void CG_RegisterClients(void)
{
	...
	{
		CG_PrecacheClientInfo(i, BG_FindModelNameForClass(i), BG_FindSkinNameForClass(i));
...
int CG_PlayerCount(void)
{
	...
	{
		if(cg.scores[i].team == PTE_ALIENS || cg.scores[i].team == PTE_HUMANS)
			count++;
	}
...
static const char *CG_FeederItemText(float feederID, int index, int column, qhandle_t * handle)
{
	...
	*handle = -1;
	...
	if((atoi(CG_ConfigString(CS_CLIENTS_READY)) & (1 << sp->client)) && cg.intermissionStarted)
		showIcons = qfalse;
