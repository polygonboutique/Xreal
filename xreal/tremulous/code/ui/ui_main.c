			...
			else
			{
				...
				else if(style == ITEM_TEXTSTYLE_NEON)
				{
					...
					Text_PaintChar(x - 1.5, y - yadj - 1.5,
								   glyph->imageWidth + 3,
								   glyph->imageHeight + 3, useScale, glyph->s, glyph->t, glyph->s2, glyph->t2, glyph->glyph);

					trap_R_SetColor(outer);
					Text_PaintChar(x - 1, y - yadj - 1,
								   glyph->imageWidth + 2,
								   glyph->imageHeight + 2, useScale, glyph->s, glyph->t, glyph->s2, glyph->t2, glyph->glyph);

					trap_R_SetColor(inner);
					Text_PaintChar(x - 0.5, y - yadj - 0.5,
								   glyph->imageWidth + 1,
								   glyph->imageHeight + 1, useScale, glyph->s, glyph->t, glyph->s2, glyph->t2, glyph->glyph);
...
qboolean UI_LoadInfoPane(int handle)
{
	...
	while(1)
	{
		...
		else if(!Q_stricmp(token.string, "graphic"))
		{
			...
			uiInfo.tremInfoPanes[uiInfo.tremInfoPaneCount].graphics[*graphic].graphic = trap_R_RegisterShaderNoMip(token.string);
...
static void UI_DrawServerRefreshDate(rectDef_t * rect, float scale, vec4_t color, int textStyle)
{
	if(uiInfo.serverStatus.refreshActive)
	{
		...
		Text_Paint(rect->x, rect->y, scale, newColor,
				   va("Getting info for %d servers (ESC to cancel)", trap_LAN_GetServerCount(ui_netSource.integer)), 0, 0,
				   textStyle);
	}
	else
	{
...
{
	uiInfo.tremAlienClassList[uiInfo.tremAlienClassCount].text = String_Alloc(BG_FindHumanNameForClassNum(class));
...
static void UI_LoadTremAlienClasses(void)
{
	...
	if(BG_ClassIsAllowed(PCL_ALIEN_BUILDER0_UPG) && BG_FindStagesForClass(PCL_ALIEN_BUILDER0_UPG, UI_GetCurrentAlienStage()))
...
{
	uiInfo.tremHumanItemList[uiInfo.tremHumanItemCount].text = String_Alloc(BG_FindHumanNameForWeapon(weapon));
	uiInfo.tremHumanItemList[uiInfo.tremHumanItemCount].cmd = String_Alloc(va("cmd class %s\n", BG_FindNameForWeapon(weapon)));
...
static void UI_LoadTremHumanItems(void)
{
	...
	if(BG_WeaponIsAllowed(WP_HBUILD2) && BG_FindStagesForWeapon(WP_HBUILD2, UI_GetCurrentHumanStage()))
...
static void UI_LoadTremHumanArmouryBuys(void)
{
	...
	for(i = WP_NONE + 1; i < WP_NUM_WEAPONS; i++)
	{
		if(BG_FindTeamForWeapon(i) == WUT_HUMANS &&
		   ...
		   BG_WeaponIsAllowed(i) && !(BG_FindSlotsForWeapon(i) & slots) && !(weapons & (1 << i)))
		{
			uiInfo.tremHumanArmouryBuyList[j].text = String_Alloc(BG_FindHumanNameForWeapon(i));
			uiInfo.tremHumanArmouryBuyList[j].cmd = String_Alloc(va("cmd buy %s retrigger\n", BG_FindNameForWeapon(i)));
			uiInfo.tremHumanArmouryBuyList[j].infopane = UI_FindInfoPaneByName(va("%sitem", BG_FindNameForWeapon(i)));
	...
	for(i = UP_NONE + 1; i < UP_NUM_UPGRADES; i++)
	{
		if(BG_FindTeamForUpgrade(i) == WUT_HUMANS &&
		   ...
		   BG_UpgradeIsAllowed(i) && !(BG_FindSlotsForUpgrade(i) & slots) && !(upgrades & (1 << i)))
		{
			uiInfo.tremHumanArmouryBuyList[j].text = String_Alloc(BG_FindHumanNameForUpgrade(i));
			uiInfo.tremHumanArmouryBuyList[j].cmd = String_Alloc(va("cmd buy %s retrigger\n", BG_FindNameForUpgrade(i)));
			uiInfo.tremHumanArmouryBuyList[j].infopane = UI_FindInfoPaneByName(va("%sitem", BG_FindNameForUpgrade(i)));
...
static void UI_LoadTremHumanArmourySells(void)
{
	...
	for(i = WP_NONE + 1; i < WP_NUM_WEAPONS; i++)
	{
		if(weapons & (1 << i))
		{
			...
			uiInfo.tremHumanArmourySellList[j].cmd = String_Alloc(va("cmd sell %s retrigger\n", BG_FindNameForWeapon(i)));
			uiInfo.tremHumanArmourySellList[j].infopane = UI_FindInfoPaneByName(va("%sitem", BG_FindNameForWeapon(i)));
	...
	for(i = UP_NONE + 1; i < UP_NUM_UPGRADES; i++)
	{
		if(upgrades & (1 << i))
		{
			...
			uiInfo.tremHumanArmourySellList[j].cmd = String_Alloc(va("cmd sell %s retrigger\n", BG_FindNameForUpgrade(i)));
			uiInfo.tremHumanArmourySellList[j].infopane = UI_FindInfoPaneByName(va("%sitem", BG_FindNameForUpgrade(i)));
...
static void UI_LoadTremAlienUpgrades(void)
{
	...
	{
		if(BG_ClassCanEvolveFromTo(class, i, credits, 0) >= 0 && BG_FindStagesForClass(i, stage) && BG_ClassIsAllowed(i))
		{
			uiInfo.tremAlienUpgradeList[j].text = String_Alloc(BG_FindHumanNameForClassNum(i));
			uiInfo.tremAlienUpgradeList[j].cmd = String_Alloc(va("cmd class %s\n", BG_FindNameForClassNum(i)));
			uiInfo.tremAlienUpgradeList[j].infopane = UI_FindInfoPaneByName(va("%sclass", BG_FindNameForClassNum(i)));
...
static void UI_LoadTremAlienBuilds(void)
{
	...
	for(i = BA_NONE + 1; i < BA_NUM_BUILDABLES; i++)
	{
		if(BG_FindTeamForBuildable(i) == BIT_ALIENS &&
		   BG_FindBuildWeaponForBuildable(i) & weapons && BG_FindStagesForBuildable(i, stage) && BG_BuildableIsAllowed(i))
		{
			...
			uiInfo.tremAlienBuildList[j].infopane = UI_FindInfoPaneByName(va("%sbuild", BG_FindNameForBuildable(i)));
...
static void UI_LoadTremHumanBuilds(void)
{
	...
	for(i = BA_NONE + 1; i < BA_NUM_BUILDABLES; i++)
	{
		   ...
		   BG_FindBuildWeaponForBuildable(i) & weapons && BG_FindStagesForBuildable(i, stage) && BG_BuildableIsAllowed(i))
		{
			...
			uiInfo.tremHumanBuildList[j].infopane = UI_FindInfoPaneByName(va("%sbuild", BG_FindNameForBuildable(i)));
...
//TA: tremulous menus
		...
		{
			trap_Cmd_ExecuteText(EXEC_APPEND,
								 va("addbot %s %i %s\n", UI_GetBotNameByNumber(uiInfo.botIndex), uiInfo.skillIndex + 1,
		...
		for(k = 0; k < uiInfo.aliasCount; k++)
		{
			if(uiInfo.aliasList[k].name != NULL)
			{
				if(Q_stricmp(uiInfo.teamList[tIndex].teamMembers[j], uiInfo.aliasList[k].name) == 0)
				{
					for(i = 0; i < uiInfo.characterCount; i++)
					{
						if(uiInfo.characterList[i].headImage != -1 && uiInfo.characterList[i].reference & (1 << tIndex) &&
	...
	else if(feederID == FEEDER_MAPS || feederID == FEEDER_ALLMAPS)
	{
		...
		if(feederID == FEEDER_MAPS)
		{
			...
			uiInfo.mapList[ui_currentMap.integer].cinematic =
				trap_CIN_PlayCinematic(va("%s.roq", uiInfo.mapList[ui_currentMap.integer].mapLoadName), 0, 0, 0, 0,
									   (CIN_loop | CIN_silent));
			UI_LoadBestScores(uiInfo.mapList[ui_currentMap.integer].mapLoadName, uiInfo.gameTypes[ui_gameType.integer].gtEnum);
		...
		for(i = 0; i < uiInfo.tremInfoPaneCount; i++)
		{
			...
			for(j = 0; j < uiInfo.tremInfoPanes[i].numGraphics; j++)
				Com_Printf("graphic %d: %d %d %d %d\n", j, uiInfo.tremInfoPanes[i].graphics[j].side,
						   ...
						   uiInfo.tremInfoPanes[i].graphics[j].width, uiInfo.tremInfoPanes[i].graphics[j].height);
		}
	}
#endif
