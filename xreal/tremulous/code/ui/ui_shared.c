...
void Item_ListBox_MouseEnter(itemDef_t * item, float x, float y)
{
	...
	if(item->window.flags & WINDOW_HORIZONTAL)
	{
		if(!
		   (item->
			window.flags & (WINDOW_LB_LEFTARROW | WINDOW_LB_RIGHTARROW | WINDOW_LB_THUMB | WINDOW_LB_PGUP | WINDOW_LB_PGDN)))
		{
	...
	if(PC_Int_Parse(handle, &num))
	{
		...
		for(i = 0; i < num; i++)
		{
			...
			if(PC_Int_Parse(handle, &pos) &&
			   PC_Int_Parse(handle, &width) && PC_Int_Parse(handle, &maxChars) && PC_Int_Parse(handle, &align))
			{
