	...
	if(totalDamage > 0.0f)
	{
		if(self->client->ps.stats[STAT_PTEAM] == PTE_ALIENS)
		{
			...
			for(i = 0; i < MAX_CLIENTS; i++)
			{
				...
				//add credit
				G_AddCreditToClient(player->client, (int)(classValue * ((float)self->credits[i] / totalDamage)), qtrue);
			}
		}
...
static float G_CalcDamageModifier(vec3_t point, gentity_t * targ, gentity_t * attacker, int class, int dflags)
{
	...
	else
	{
		for(i = 0; i < g_numDamageRegions[class]; i++)
		{
			...
			if(g_damageRegions[class][i].minAngle > g_damageRegions[class][i].maxAngle)
			{
				rotationBound = (hitRotation >= g_damageRegions[class][i].minAngle &&
								 hitRotation <= 360) || (hitRotation >= 0 && hitRotation <= g_damageRegions[class][i].maxAngle);
			}
			else
			{
				rotationBound = (hitRotation >= g_damageRegions[class][i].minAngle &&
		...
		for(i = UP_NONE + 1; i < UP_NUM_UPGRADES; i++)
		{
			if(BG_InventoryContainsUpgrade(i, targ->client->ps.stats))
			{
				for(j = 0; j < g_numArmourRegions[i]; j++)
				{
					...
					if(g_armourRegions[i][j].minAngle > g_armourRegions[i][j].maxAngle)
					{
			  ...
			  vec3_t dir, vec3_t point, int damage, int dflags, int mod)
{
	...
	if(client)
	{
		...
		take = (int)((float)take * G_CalcDamageModifier(point, targ, attacker, client->ps.stats[STAT_PCLASS], dflags));
