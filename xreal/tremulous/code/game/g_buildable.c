...
static qboolean findPower(gentity_t * self)
{
	...
	if((closestPower->s.modelindex == BA_H_REACTOR && (minDistance <= REACTOR_BASESIZE)) ||
	   (closestPower->s.modelindex == BA_H_REPEATER && (minDistance <= REPEATER_BASESIZE) && closestPower->powered))
	{
...
void ASpawn_Think(gentity_t * self)
{
	...
	if(self->spawned)
	{
		...
		if(self->s.groundEntityNum)
		{
			...
			{
				if(ent->s.eType == ET_BUILDABLE || ent->s.number == ENTITYNUM_WORLD || ent->s.eType == ET_MOVER)
				{
...
================
*/
static qboolean APropHovel_Blocked(vec3_t origin, vec3_t angles, vec3_t normal, gentity_t * player)
{
	gentity_t       hovel;
...
void HReactor_Think(gentity_t * self)
{
	...
	if(self->spawned && (self->health > 0))
	{
		...
		if(self->health < self->lastHealth && level.time > level.humanBaseAttackTimer && G_isDCC())
		{
...
void HTeslaGen_Think(gentity_t * self)
{
	...
	if(self->spawned && self->count < level.time)
	{
		...
		for(i = 0; i < num; i++)
		{
			...
			if(enemy->client && enemy->client->ps.stats[STAT_PTEAM] == PTE_ALIENS && enemy->health > 0)
			{
...
void HSpawn_Think(gentity_t * self)
{
	...
	if(self->spawned)
	{
		...
		if(self->s.groundEntityNum)
		{
			if((ent = G_CheckSpawnPoint(self->s.number, self->s.origin, self->s.origin2, BA_H_SPAWN, NULL)) != NULL)
			{
				if(ent->s.eType == ET_BUILDABLE || ent->s.number == ENTITYNUM_WORLD || ent->s.eType == ET_MOVER)
				{
					G_Damage(self, NULL, NULL, NULL, NULL, 10000, 0, MOD_SUICIDE);
...
gentity_t      *G_buildItem(gentity_t * builder, buildable_t buildable, vec3_t origin, vec3_t angles)
{
	...
	built->s.generic1 = (int)(((float)built->health / (float)BG_FindHealthForBuildable(buildable)) * B_HEALTH_SCALE);
