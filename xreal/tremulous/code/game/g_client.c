...
void G_AddCreditToClient(gclient_t * client, short credit, qboolean cap)
{
	...
	if(cap)
	{
		...
		{
			if(client->ps.persistant[PERS_CREDIT] >= ALIEN_MAX_KILLS && credit > 0)
				return;
		}
		else if(client->ps.stats[STAT_PTEAM] == PTE_HUMANS)
		{
			if(client->ps.persistant[PERS_CREDIT] >= HUMAN_MAX_CREDITS && credit > 0)
				return;
		}
	}
...
gentity_t      *SelectAlienSpawnPoint(vec3_t preference)
{
	...
	while((spot = G_Find(spot, FOFS(classname), BG_FindEntityNameForBuildable(BA_A_SPAWN))) != NULL)
	{
		if(!spot->spawned)
			continue;
		...
		if(G_CheckSpawnPoint(spot->s.number, spot->s.origin, spot->s.origin2, BA_A_SPAWN, NULL) != NULL)
			continue;
...
void ClientUserinfoChanged(int clientNum)
{
	...
	s = va
		("n\\%s\\t\\%i\\model\\%s\\hmodel\\%s\\g_redteam\\%s\\g_blueteam\\%s\\c1\\%s\\c2\\%s\\hc\\%i\\w\\%i\\l\\%i\\tt\\%d\\tl\\%d",
		 client->pers.netname, team, model, model, redTeam, blueTeam, c1, c2, client->pers.maxHealth, client->sess.wins,
...
#define F_VEL   50.0f
	...
	else if(client->sess.sessionTeam != TEAM_SPECTATOR && client->ps.stats[STAT_PTEAM] == PTE_HUMANS)
	{
		spawn_angles[YAW] += 180.0f;
		AngleNormalize360(spawn_angles[YAW]);
	}
