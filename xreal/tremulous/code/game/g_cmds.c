...
void ScoreboardMessage(gentity_t * ent)
{
	...
	G_SendCommandFromServer(ent - g_entities, va("scores %i %i %i%s", i, level.alienKills, level.humanKills, string));
}
...
void Cmd_CallVote_f(gentity_t * ent)
{
	...
	else
	{
		...
		G_SendCommandFromServer(ent - g_entities, "print \"Vote commands are: map_restart, nextmap, map <mapname>, "
								"kick <player>, clientkick <clientnum>, " "timelimit <time>\n\"");
		return;
	}
...
void Cmd_CallTeamVote_f(gentity_t * ent)
{
	...
	Com_sprintf(level.teamVoteString[cs_offset], sizeof(level.teamVoteString[cs_offset]), "kick \"%s\"", arg2);
...
void Cmd_Class_f(gentity_t * ent)
{
	...
	if(BG_ClassIsAllowed(PCL_ALIEN_BUILDER0_UPG) && BG_FindStagesForClass(PCL_ALIEN_BUILDER0_UPG, g_alienStage.integer))
	   ...
	   !(ent->client->ps.stats[STAT_STATE] & SS_WALLCLIMBING) && !(ent->client->ps.stats[STAT_STATE] & SS_WALLCLIMBINGCEILING))
	{
		...
		if(currentClass != PCL_NONE)
		{
			...
			if((currentClass == PCL_ALIEN_BUILDER0 ||
				currentClass == PCL_ALIEN_BUILDER0_UPG) && ent->client->ps.stats[STAT_MISC] > 0)
			{
			...
			BG_FindBBoxForClass(currentClass, fromMins, fromMaxs, NULL, NULL, NULL);
			BG_FindBBoxForClass(ent->client->pers.classSelection, toMins, toMaxs, NULL, NULL, NULL);
			...
			if(!tr.startsolid && tr2.fraction == 1.0f)
			{
				...
				{
					ent->client->pers.classSelection = PCL_NONE;
					G_SendCommandFromServer(ent - g_entities, va("print \"You cannot evolve from your current class\n\""));
					return;
				}
			}
			else
		...
		else
		{
			...
			ent->client->pers.classSelection = ent->client->ps.stats[STAT_PCLASS] = BG_FindClassNumForName(s);
	...
	else if(ent->client->pers.teamSelection == PTE_HUMANS)
	{
		...
		ent->client->pers.classSelection = ent->client->ps.stats[STAT_PCLASS] = PCL_HUMAN;
...
void Cmd_Buy_f(gentity_t * ent)
{
	...
	if(weapon != WP_NONE)
	{
		...
		if(BG_FindUsesEnergyForWeapon(weapon) && BG_InventoryContainsUpgrade(UP_BATTPACK, ent->client->ps.stats))
			maxAmmo = (int)((float)maxAmmo * BATTPACK_MODIFIER);

		BG_PackAmmoArray(weapon, ent->client->ps.ammo, ent->client->ps.powerups, maxAmmo, maxClips);
...
void Cmd_Sell_f(gentity_t * ent)
{
	...
	if(weapon != WP_NONE)
	{
		...
		{
			//guard against selling the HBUILD weapons exploit
			if((weapon == WP_HBUILD || weapon == WP_HBUILD2) && ent->client->ps.stats[STAT_MISC] > 0)
			{
	...
	else if(!Q_stricmp(s, "weapons"))
	{
		...
		{
			//guard against selling the HBUILD weapons exploit
			if((i == WP_HBUILD || i == WP_HBUILD2) && ent->client->ps.stats[STAT_MISC] > 0)
			{
				G_SendCommandFromServer(ent - g_entities, va("print \"Cannot sell until build timer expires\n\""));
				continue;
			}

			if(BG_InventoryContainsWeapon(i, ent->client->ps.stats) && BG_FindPurchasableForWeapon(i))
			{
	...
	else if(!Q_stricmp(s, "upgrades"))
	{
		...
		{
			//remove upgrade if carried
			if(BG_InventoryContainsUpgrade(i, ent->client->ps.stats) && BG_FindPurchasableForUpgrade(i))
			{
...
{
	if(BG_InventoryContainsUpgrade(UP_JETPACK, ent->client->ps.stats) && BG_UpgradeIsActive(UP_JETPACK, ent->client->ps.stats))
		return;

	if(ent->client->pers.cmd.buttons & BUTTON_WALKING)
		return;

	if((ent->client->ps.stats[STAT_PTEAM] == PTE_HUMANS) && (ent->client->ps.stats[STAT_STAMINA] > 0))
...
void Cmd_PTRCVerify_f(gentity_t * ent)
{
	...
	else
	{
		...
		if(connection)
		{
			G_SendCommandFromServer(ent->client->ps.clientNum, va("ptrcissue %d", connection->ptrCode));
		}
	}
}
...
void Cmd_PTRCRestore_f(gentity_t * ent)
{
	...
	if(G_VerifyPTRC(code))
	{
		...
		{
			G_SendCommandFromServer(ent - g_entities, "print \"You cannot use a PTR code after joining a team\n\"");
		}
		else
		{
			// valid code
	...
	{
		G_SendCommandFromServer(ent - g_entities, va("print \"\"%d\" is not a valid PTR code\n\"", code));
	}
}
