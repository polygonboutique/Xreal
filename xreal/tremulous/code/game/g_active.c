	...
	while(client->time1000 >= 1000)
	{
		...
		if(client->ps.stats[STAT_PTEAM] == PTE_ALIENS)
		{
			...
			for(i = 0; i < num; i++)
			{
				...
				else if(boostEntity->s.eType == ET_BUILDABLE && boostEntity->s.modelindex == BA_A_BOOSTER && boostEntity->spawned)
				{
...
//    G_Printf("serverTime >>>>>\n" );
	...
	if(client->ps.stats[STAT_STATE] & SS_BLOBLOCKED && client->lastLockTime + 5000 < level.time)
		client->ps.stats[STAT_STATE] &= ~SS_BLOBLOCKED;

	if(client->ps.stats[STAT_STATE] & SS_SLOWLOCKED && client->lastLockTime + ABUILDER_BLOB_TIME < level.time)
...
#define USE_OBJECT_RANGE 64
	...
	if(client->ps.stats[STAT_HEALTH] <= 0)
	{
		...
		if(level.time > client->respawnTime)
		{
			...
			if(g_forcerespawn.integer > 0 && (level.time - client->respawnTime) > 0)
			{
				respawn(ent);
				return;
			}
	...
	{
		if(g_suddenDeathTime.integer && (level.time - level.startTime >= g_suddenDeathTime.integer * 60000))
		{
