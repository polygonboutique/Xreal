...
static void CG_HasteTrail( centity_t *cent ,vec3_t head[3],vec3_t legs[3],vec3_t torso[3],
	...
	if(cent->currentState.legsAnim == LEGS_IDLECR){
		cent->trailTime = cg.time;
		return;
	}else{
		...
		for (  ; t <= cent->trailTime ; t += step) {
			...
			CG_HasteFX( cent,lastPos,t,head,legs,torso,
						  legframe,torsoframe,headOrg,torsoOrg);
...
static void CG_BreathPuffs( centity_t *cent, refEntity_t *head) {
	...
	if ( anim == LEGS_RUN || anim == LEGS_BACK ) {
		...
		CG_SmokePuff( origin, velocity, 5.5, 1, 1, 1, 0.35f, 350, cg.time, cg.time + 50, LEF_PUFF_DONT_SCALE, cgs.media.shotgunSmokePuffShader );
		CG_SmokePuff( origin, velocity2, 5, 1, 1, 1, 0.35f, 350, cg.time, cg.time + 50, LEF_PUFF_DONT_SCALE, cgs.media.shotgunSmokePuffShader );
		...
		CG_SmokePuff( origin, velocity, 4.5, 1, 1, 1, 0.55f, 800, cg.time, cg.time + 200, LEF_PUFF_DONT_SCALE, cgs.media.shotgunSmokePuffShader );
		CG_SmokePuff( origin, velocity2, 4, 1, 1, 1, 0.55f, 700, cg.time, cg.time + 200, LEF_PUFF_DONT_SCALE, cgs.media.shotgunSmokePuffShader );
		...
		CG_SmokePuff( origin, velocity, 4.5, 1, 1, 1, 0.65f, 1200, cg.time, cg.time + 500, LEF_PUFF_DONT_SCALE, cgs.media.shotgunSmokePuffShader );
		CG_SmokePuff( origin, velocity2, 4, 1, 1, 1, 0.65f, 1100, cg.time, cg.time + 400, LEF_PUFF_DONT_SCALE, cgs.media.shotgunSmokePuffShader );
		...
		CG_SmokePuff( origin, velocity, 5.5f, 1, 1, 1, 0.35f, 200, cg.time, cg.time + 20, LEF_PUFF_DONT_SCALE, cgs.media.shotgunSmokePuffShader );
		CG_SmokePuff( origin, velocity2, 5, 1, 1, 1, 0.35f, 200, cg.time, cg.time + 20, LEF_PUFF_DONT_SCALE, cgs.media.shotgunSmokePuffShader );
...
void CG_DeadPlayerHead( centity_t *cent ) {
	...
	CG_PlayerAnimation( cent, &legs.oldframe, &legs.frame, &legs.backlerp,
		 &torso.oldframe, &torso.frame, &torso.backlerp ,qfalse);
...
//	vec3_t	newOrigin;
	...
	for ( ; t <= t2; t += step ) {
		...
		CG_Particle_Fire (cgs.media.FireParticleShader
			, Origin
			, vec3_origin
			, 0
			, 50+rand()%400
			, startsize,endsize,randorg);
	}
}
...
//	vec3_t	newOrigin;
	...
	for ( ; t <= t2; t += step ) {
		...
		CG_Particle_FireSparks (cgs.media.FireSparksShader
			, Origin
			, vec3_origin
			, 0
			, 450+rand()%400
...
//	vec3_t	newOrigin;
	...
	for ( ; t <= t2; t += step ) {
		...
		CG_Particle_FireSmoke (cgs.media.BLACKSmokeShader
			, Origin
			, vec3_origin
			, 0
			, 650+rand()%400
...
void CG_Player( centity_t *cent ) {
	...
	//
		...
		if ( cent->currentState.powerups & ( 1 << PW_HASTE ) &&
			...
			CG_HasteTrail( cent ,head.axis,legs.axis,torso.axis,
				legs.frame,torso.frame,head.origin,torso.origin);
		}
