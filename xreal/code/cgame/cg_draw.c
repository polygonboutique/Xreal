...
//	}
	...
	CG_DrawHead( x, y - size, size/2, size/2, 
...
static void CG_DrawStatusBar( void ) {
	...
	if( cg.predictedPlayerState.powerups[PW_BLUEFLAG] ) {
		CG_DrawStatusBarFlag( 42, TEAM_BLUE );
			...
			CG_Draw3DModel( 345, 428, ICON_SIZE/2, ICON_SIZE/2,
...
static void CG_DrawReward( void ) { 
	...
	*/
	...
	//	CG_DrawPic( x, y, ICON_SIZE-4, ICON_SIZE-4, cg.rewardShader[0] );
		...
		CG_Draw3DModel( x, y, ICON_SIZE, ICON_SIZE,
						   cg.rewardModel[0], 0, origin, angles );
...
void CG_DrawCrosshair(void) {
	...
	trap_R_DrawStretchPic( x + cg.refdef[0].x + 0.5 * (cg.refdef[0].width - w), 
		y + cg.refdef[0].y + 0.5 * (cg.refdef[0].height - h), 
		w, h, 0, 0, 1, 1, hShader );
}
...
//	vec_t		alpha2;
	...
	if ( cg.logoTime1 >= 6000 ) {
		...
		trap_R_SetColor( NULL );
		 ...
		 CH_DrawStringExt( x+75, 326, "^b^0**** ^1Xreal Version ^41.0b ^0****", 6, 10, colorWhite, 85, qfalse);

		 CH_DrawStringExt( x+15, 338, "", 6, 10, colorWhite, 85, qfalse);

		motd1 = va("%s", cgs.motd1);
		motd2 = va("%s", cgs.motd2);
		motd3 = va("%s", cgs.motd3);
		// motd #1
...
void CG_DrawStatsFinal( void ) {
	...
	if ( cg.statTime >= 6000 ) {
		x = ( cg.statTime - 6000 ) * -300 / 2000 + 10;
	  	...
	  	CG_DrawPic( x+565, 325+50,35 , 85, trap_R_RegisterShaderNoMip( "gfx/stats/q3qslogo01" ) );
	...
	// Total Health + Armor Pickup
		CH_DrawStringExt( x+312, 358+50, "^F^1Total Health Recieved^f^3^xff0011Total Health Recieved^N:", 6, 7, colorWhite, 85, qfalse);
			pat = va("^B^xff0011^3%i ", c2->pickupArmorT);
		CH_DrawStringExt( x+444, 358+50, pat, 6, 7, colorWhite, 85, qfalse);

		CH_DrawStringExt( x+312, 365+50, "^F^1Total Armor Recieved^f^3^xff0011Total Armor Recieved^N:", 6, 7, colorWhite, 85, qfalse);
	...
	// Damage
		CH_DrawStringExt( x+312, 393+50, "^F^1Total Damage Recieved^f^3^xff0011Total Damage Recieved^N:", 6, 7, colorWhite, 85, qfalse);
