...
static void CG_BloodTrail(localEntity_t * le, vec3_t vel)
{
	...
	for(; t <= t2; t += step)
	{
		...
		CG_Particle_Bleed(hshader, newOrigin, vec3_origin, 0, 1000 + rand() % 400, 7, 17 + rand() % 10);
	}
}
	...
	if(contents & (CONTENTS_WATER | CONTENTS_SLIME | CONTENTS_LAVA))
	{
		...
		if(contents & lastContents & CONTENTS_WATER)
		{
			if(le->inwater && le->watertraced && cg.time > le->startTime + 5)
			{
				...
				{
					CG_AddBulletSplashParticles(lastPos, trace2.plane.normal, 230,	// speed
												700,	// duration
												3,	// count
...
static void CG_FragmentBounceMark(localEntity_t * le, trace_t * trace)
{
	...
	if(le->leMarkType == LEMT_BLOOD)
	{
		...
		radius = 16 + (rand() & 31);
		CG_ImpactMark(hshader, trace->endpos, trace->plane.normal, random() * 360, 1, 1, 1, 1, qtrue, radius, qfalse);
	}
...
void CG_AddFadeRGBA(localEntity_t * le)
{
	...
	if(re->reType == RT_RAIL_CORE)
	{
		...
		{
			//      CG_DynamicLightningBolt( cgs.media.lightningShader, re->origin, re->oldorigin, 1, 1 , qfalse, 1.0, 1, 1 ,qfalse,qtrue);
		}
	}
	trap_R_AddRefEntityToScene(re);
}
...
void CG_FlarePuff(const vec3_t p, const vec3_t vel,
				  ...
				  float duration, int startTime, int fadeInTime, int leFlags, qhandle_t hShader, char *animStr, qboolean animated)
{
...
void CG_RLSMOKE(const vec3_t p, const vec3_t vel,
				float radius,
				float r, float g, float b, float a, float duration, int startTime, int fadeInTime, int leFlags, char *animStr)
{
	static int      seed = 0x92;
	int             anim;
	localEntity_t  *le;
...
void CG_RocketExplosion(vec3_t dir, vec3_t org, char *animStr, int duration, float light, vec3_t lightColor, float light2,
						vec3_t lightColor2)
{
	localEntity_t  *le;
	refEntity_t    *re;
...
void CG_GrenadeExplosion(vec3_t dir, vec3_t org, char *animStr, int duration, float light, vec3_t lightColor, float light2,
						 vec3_t lightColor2)
{
	localEntity_t  *le;
	refEntity_t    *re;
...
static void CG_AddRailParticleEXP(vec3_t origin, vec3_t dir, int count, int duration, int speed, float startsize, float endsize)
{
	localEntity_t  *le;
	refEntity_t    *re;
	vec3_t          velocity;
...
static void CG_SparkTrail(localEntity_t * le, int trailtime, qhandle_t hShader, int lstep, float startsize, float endsize)
{
	static vec3_t   col = { 1, 1, 1 };
	int             t;
	int             t2;
...
static void CG_AddRailSpark(localEntity_t * le)
{
	...
	if(trace.fraction == 1.0)
	{
		if(le->pos.trType != TR_STATIONARY)
		{
			CG_SparkTrail(le, le->trailLength, le->hShader, le->step, le->startSize, le->endSize);
		}
		VectorCopy(newOrg, le->refEntity.origin);
		return;
	}
	...
	{
		CG_AddRailParticleEXP(re->origin, le->pos.trDelta, 5, 600, 70, 3, 1);
		CG_FreeLocalEntity(le);
		return;
	}
