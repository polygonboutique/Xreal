...
static void CG_AddRailParticles( clientInfo_t *ci, vec3_t origin, vec3_t dir,int count ,int duration,
								  ...
								  float	startsize,float	endsize,vec3_t ma) {
	localEntity_t	*le;
	refEntity_t		*re;
	vec3_t	velocity;
	int	i;  
	...
	for ( ; t <= ent->trailTime ; t += step ) {
		...
		if(cgs.InstaGib == 0){
			 CG_FlarePuff( lastPos, 0, 
							  ...
							  t,
							  0,
							  LEF_PUFF_DONT_SCALE/*|LEF_NO_RGB_FADE*/, 
	...
	for ( ; t2 <= ent->trailTime ; t2 += step2 ) {
		...
		if(cgs.InstaGib == 0){
			CG_FlarePuff( lastPos, 0, 
							  ...
							  0,
							  LEF_PUFF_DONT_SCALE, 
							  cgs.media.PLSmokeShader,"" ,qfalse);
		}else{
			CG_FlarePuff( lastPos, 0, 
							  8.5f, 
							  0.8f, 0.8f, 0.3f, 1,
	...
	//	if ( contents & ( CONTENTS_WATER | CONTENTS_SLIME | CONTENTS_LAVA ) ) {
			if(cgs.gametype >= GT_TEAM){
				...
				CG_FlarePuff( lastPos, up, 
								  ...
								  0,
								  0, 
								  cgs.media.LIGHTSmokeRingShader ,"",qfalse);
				CG_FlarePuff( lastPos, up, 
								  0.5f+rand()%1, 
								  1, 0.5f, 0.5f, 1,
								  300+rand()%100, 
								  ...
								  0,
								  0, 
								  cgs.media.GREYSmokeRingShader ,"",qfalse);
...
static void CG_PlasmaRing( const vec3_t p, const vec3_t vel, 
				   float duration,
				   int startTime,
				   vec3_t axis[3],
				   int team) {
	...
	if ( cent->currentState.weapon == WP_MACHINEGUN || WP_GAUNTLET) {
		if ( cent->pe.barrelSpinning == !(cent->currentState.eFlags & EF_FIRING) ) {
			...
			if ( cent->currentState.weapon == WP_MACHINEGUN && !cent->pe.barrelSpinning ) {
				trap_S_StartSound( NULL, cent->currentState.number, CHAN_WEAPON, trap_S_RegisterSound( "sound/weapons/machinegun/winddown.wav", qfalse ) );
			}
...
//	flash.shadowPlane = parent->shadowPlane;
	...
	if(weaponNum != WP_IRAILGUN && weaponNum != WP_RAILGUN) {	//Ridah, hide the flash also for now
		...
		if(cg_QSLights.integer == 1){
			if ( weapon->flashDlightColor[0] || weapon->flashDlightColor[1] || weapon->flashDlightColor[2] ) {
				if ( weaponNum == WP_PLASMAGUN){
					...
					if(cgs.gametype >= GT_TEAM){
						if(ci->team == TEAM_RED){
							trap_R_AddAdditiveLightToScene(  flash.origin, 110, 0.2f,
								0.05f, 0 );
							trap_R_AddLightToScene(  flash.origin, 130, 0.9f,
								0.2f, 0);
							...
							trap_R_AddAdditiveLightToScene(  flash.origin, 110, 0.05f,
								0.2f,0 );
							trap_R_AddLightToScene(  flash.origin, 130, 0.2f,
								0.9f, 0 );
					}
				}else{
					trap_R_AddAdditiveLightToScene(  flash.origin, 110, weapon->flashDlightColor[0],
...
void CG_DrawWeaponSelectModel( float x, float y, float w, float h, weaponInfo_t	*weapon ) {
	...
	origin[0] -= 
		...
		weapon->weaponMidpoint[1] * ent.axis[1][0] +
		weapon->weaponMidpoint[2] * ent.axis[2][0];
	origin[1] -= 
		weapon->weaponMidpoint[0] * ent.axis[0][1] +
		weapon->weaponMidpoint[1] * ent.axis[1][1] +
		weapon->weaponMidpoint[2] * ent.axis[2][1];
	origin[2] -= 
		weapon->weaponMidpoint[0] * ent.axis[0][2] +
		weapon->weaponMidpoint[1] * ent.axis[1][2] +
		weapon->weaponMidpoint[2] * ent.axis[2][2];
...
void CG_RailgunPrefire( centity_t *cent ) {
	...
	if (ent->weapon == WP_IRAILGUN){
		if (charge > 0){
			trap_R_AddAdditiveLightToScene(  ent->origin, brightness + (rand()&15), color[0],
			color[1], color[2] );
		}
	}
...
void CG_MissileHitWall( int weapon, int clientNum, vec3_t origin, vec3_t dir, impactSound_t soundType,qboolean quad,qboolean player) {
		...
		case WP_PLASMAGUN:
			if(weapon == WP_GRENADE_LAUNCHER){
				...
				if(finaltrace.fraction != 1.0 ){
					if(quad){
						CG_AddBulletSplashParticles( finaltrace.endpos, finaltrace.plane.normal,
												290,	// speed
												1000,	// duration
												5,	// count
	...
	case WP_GRENADE_LAUNCHER:
		...
		if(cg_QSLights.integer == 1){
			...
			CG_GrenadeExplosion( dir, origin ,
				"rlexplosion1",
				800,
				light2,
				lightColor,
	...
	case WP_SHOTGUN:
		...
		le = CG_MakeExplosion( sprOrg, dir, 
							   0,	cgs.media.bulletExplosionShader,
							   150, qtrue , 3);
		le->light = 0;
		le->light2 = 0;
		le->width = radius;
	...
	if ( mod ) {
		if(weapon == WP_GRENADE_LAUNCHER){
			...
			le = CG_MakeExplosion( spror, dir, 
								   mod,	shader,
								   duration, isSprite , radius);
		}else{
...
static void CG_ShotgunPellet( vec3_t start, vec3_t end, int skipNum ) {
	...
	//	CG_AddPlasmaParticles(trace.endpos,trace.plane.normal,100,TEAM_FREE);
		trap_S_StartSound( trace.endpos, ENTITYNUM_WORLD, CHAN_AUTO, cgs.media.waterHitsmallSound );
				...
				CG_AddBulletSplashParticles( trace.endpos, trace.plane.normal,
								170,	// speed
								400,	// duration
								2,	// count
