...
void CG_AddMarks(void)
{
	...
	for(; mp != &cg_activeMarkPolys; mp = next)
	{
		...
		if(mp->markShader == cgs.media.energyMarkShader ||
		   mp->markShader == cgs.media.energyMarkrShader ||
		   mp->markShader == cgs.media.energyMarkgShader || cgs.media.energy1MarkShader)
		{
		...
		if(t < MARK_FADE_TIME)
		{
			   ...
			   mp->markShader == cgs.media.energyMarkgShader || cgs.media.energy1MarkShader)
			{
				for(j = 0; j < mp->poly.numVerts; j++)
				{
					mp->verts[j].modulate[0] = 0;
					mp->verts[j].modulate[1] = 0;
					mp->verts[j].modulate[2] = 0;
				}
			}
			else
			{
		...
		if(mp->markShader == cgs.media.energyMarkShader ||
		   mp->markShader == cgs.media.energyMarkrShader ||
		   mp->markShader == cgs.media.energyMarkgShader || mp->markShader == cgs.media.energy1MarkShader)
		{
			// let the mark stay its full time
		}
		else
		{
			if(mp->markShader != cgs.media.bloodMarkShader &&
			   mp->markShader != cgs.media.zbloodSplat1Shader &&
			   mp->markShader != cgs.media.zbloodSplat2Shader && mp->markShader != cgs.media.zbloodSplat3Shader)
			{
				if(cg.time > mp->time + 4000)
				{
					CG_FreeMarkPoly(mp);
					//  continue;
				}
...
vec3_t          srforward, srright, srup;
vec3_t          spforward, spright, spup;
float           oldtime;
...
=====================
*/
void CG_AddParticleToScene(cparticle_t * p, vec3_t org, float alpha, int view)
{
	...
	else if(p->type == P_SPRITE)
	{
		...
		if(p->roll)
		{
			vectoangles(cg.refdef[view].viewaxis[0], rotate_ang);
	...
	{							// create a front rotating facing polygon
		...
		if(p->color == BLOODRED)
		{
			VectorSet(color, 0.92f, 0.0f, 0.0f);
		}
		else if(p->color == GREEN)
		{
			VectorSet(color, 0.0f, 0.92f, 0.0f);
		}
		else if(p->color == BLUE)
		{
			VectorSet(color, 0.0f, 0.0f, 0.92f);
		}
		...
		{
			VectorSet(color, BLACK, BLACK, BLACK);
		}
		else
		{
		...
		AngleVectors(temp, NULL, NULL, up);

		//  VectorCopy (org,point);
		VectorMA(org, -p->height / 2, up, point);
		VectorMA(point, -p->width, pvright, point);
		...
		VectorMA(org, p->height, up, point);
		VectorMA(point, p->width, pvright, point);
		...
		// add the crossover poly so it looks full from top and bottom view
		VectorMA(org, -(p->width + p->width / 1.5f), pvup, point);
		VectorMA(point, -(p->width + p->width / 1.5f), pvright, point);
		VectorCopy(point, verts2[0].xyz);
		verts2[0].st[0] = 0;
		verts2[0].st[1] = 0;
		verts2[0].modulate[0] = 255 * color[0];
		verts2[0].modulate[1] = 255 * color[1];
		verts2[0].modulate[2] = 255 * color[2];
		verts2[0].modulate[3] = 255 * invratio;

		VectorMA(org, -(p->width + p->width / 1.5f), pvup, point);
		VectorMA(point, (p->width + p->width / 1.5f), pvright, point);
		VectorCopy(point, verts2[1].xyz);
		verts2[1].st[0] = 0;
		verts2[1].st[1] = 1;
		verts2[1].modulate[0] = 255 * color[0];
		verts2[1].modulate[1] = 255 * color[1];
		verts2[1].modulate[2] = 255 * color[2];
		verts2[1].modulate[3] = 255 * invratio;

		VectorMA(org, (p->width + p->width / 1.5f), pvup, point);
		VectorMA(point, (p->width + p->width / 1.5f), pvright, point);
		VectorCopy(point, verts2[2].xyz);
		verts2[2].st[0] = 1;
		verts2[2].st[1] = 1;
		verts2[2].modulate[0] = 255 * color[0];
		verts2[2].modulate[1] = 255 * color[1];
		verts2[2].modulate[2] = 255 * color[2];
		verts2[2].modulate[3] = 255 * invratio;

		VectorMA(org, (p->width + p->width / 1.5f), pvup, point);
		VectorMA(point, -(p->width + p->width / 1.5f), pvright, point);
		VectorCopy(point, verts2[3].xyz);
		verts2[3].st[0] = 1;
		verts2[3].st[1] = 0;
		verts2[3].modulate[0] = 255 * color[0];
		verts2[3].modulate[1] = 255 * color[1];
		verts2[3].modulate[2] = 255 * color[2];
		verts2[3].modulate[3] = 255 * invratio;
	...
	{							// create a front rotating facing polygon
		...
		if(p->type != P_SMOKE_IMPACT)
		{
			...
			vectoangles(srforward, temp);
			p->accumroll += p->roll;
			temp[ROLL] += p->accumroll * 0.1;
			AngleVectors(temp, NULL, srright2, srup2);
		}
		else
		{
			VectorCopy(srright, srright2);
			VectorCopy(srup, srup2);
		}
...
/*
		if (p->roll) {
			VectorMA (org, -2*height, ru, point);	
			VectorMA (point, -2*width, pvright, point);	
		} else {
			VectorMA (org, -2*height, pvup, point);	
			VectorMA (point, -2*width, pvright, point);
		}
		VectorCopy( point, TRIverts[0].xyz );
		TRIverts[0].st[0] = -1;
		TRIverts[0].st[1] = -1;
		TRIverts[0].modulate[0] = 255 * color[0];
		TRIverts[0].modulate[1] = 255 * color[1];
		TRIverts[0].modulate[2] = 255 * color[2];
		TRIverts[0].modulate[3] = 255 * invratio;	
		...
		TRIverts[1].st[0] = 0;
		TRIverts[1].st[1] = 0;
		TRIverts[1].modulate[0] = 255 * color[0];
		TRIverts[1].modulate[1] = 255 * color[1];
		TRIverts[1].modulate[2] = 255 * color[2];
		TRIverts[1].modulate[3] = 255 * invratio;	
		...
		TRIverts[2].st[0] = 0;
		TRIverts[2].st[1] = 0;
		TRIverts[2].modulate[0] = 255 * color[0];
		TRIverts[2].modulate[1] = 255 * color[1];
		TRIverts[2].modulate[2] = 255 * color[2];
		TRIverts[2].modulate[3] = 255 * invratio;	
		*/
...
//      CG_Printf ("CG_AddParticleToScene type %d p->pshader == ZERO\n", p->type);
	...
	else
	{
		if(p->type == P_SPLASH)
		{
			trap_R_AddPolyToScene(p->pshader, 4, verts2);
		}
...
void CG_AddParticles(int view)
{
	...
	VectorCopy(cg.refdef[view].viewaxis[0], pvforward);
	VectorCopy(cg.refdef[view].viewaxis[1], pvright);
	VectorCopy(cg.refdef[view].viewaxis[2], pvup);
	vectoangles(cg.refdef[view].viewaxis[0], rotate_ang);


	roll += ((cg.time - oldtime) * 0.1);
	rotate_ang[ROLL] += (roll * 0.9);
	AngleVectors(rotate_ang, srforward, srright, srup);
	...
	for(p = active_particles; p; p = next)
	{
		...
		if(p->type == P_SMOKE || p->type == P_ANIM || p->type == P_BLEED || p->type == P_SMOKE_IMPACT || p->type == P_SPLASH)
		{
		...
		{
			// temporary sprite
			CG_AddParticleToScene(p, p->org, alpha, view);
			p->next = free_particles;
			free_particles = p;
			p->type = 0;
			p->color = 0;
		...
		CG_AddParticleToScene(p, org, alpha, view);
	}
...
//  int i;  
	...
	x = cg_SGParticles.integer;
	if(!free_particles)
		return;
	speed = 30 + random() * 35;
	duration = 900 + random() * 1300;
	p = free_particles;
	free_particles = p->next;
	p->next = active_particles;
	active_particles = p;
	...
	VectorSet(vel, dir[0] + crandom(), dir[1] + crandom(), dir[2] + crandom());
	VectorScale(vel, (float)speed * 3, vel);
	...
	VectorMA(p->org, 2 + random() * 2, dir, p->org);
...
void CG_Particle_Fire(qhandle_t pshader, vec3_t start, vec3_t dir, int fleshEntityNum, int duration, int size, int endsize,
					  int randorg)
{
	cparticle_t    *p;
	int             i;
	...
	if(i == 0)
	{
		p->pshader = cgs.media.smallFire1;
	}
	else if(i == 1)
	{
		p->pshader = cgs.media.smallFire2;
	}
	else if(i == 2)
	{
		p->pshader = cgs.media.smallFire3;
	}
	else if(i == 3)
	{
		p->pshader = cgs.media.smallFire4;
	}
	else if(i == 4)
	{
		p->pshader = cgs.media.smallFire5;
	}
	else if(i == 5)
	{
		p->pshader = cgs.media.smallFire6;
	}
	else if(i == 6)
	{
		p->pshader = cgs.media.smallFire7;
	}
	else if(i == 7)
	{
		p->pshader = cgs.media.smallFire8;
	}
	else if(i == 8)
	{
		p->pshader = cgs.media.smallFire9;
	}
	else if(i == 9)
	{
		p->pshader = cgs.media.smallFire10;
	}
	else if(i == 10)
	{
		p->pshader = cgs.media.smallFire11;
	}
	else if(i == 11)
	{
		p->pshader = cgs.media.smallFire12;
	}
	else if(i == 12)
	{
		p->pshader = cgs.media.smallFire13;
	}
	else if(i == 13)
	{
		p->pshader = cgs.media.smallFire14;
	}
	else if(i == 14)
	{
		p->pshader = cgs.media.smallFire15;
	}
	else if(i == 15)
	{
		p->pshader = cgs.media.smallFire16;
	}
	else if(i == 16)
	{
		p->pshader = cgs.media.smallFire17;
	}
...
//  CG_RocketCorona( start ,5,size);

	VectorCopy(start, p->org);
	p->org[0] += rand() % randorg;
	p->org[1] += rand() % randorg;
	p->org[2] += rand() % randorg;
...
void CG_Particle_FireTeam(qhandle_t pshader, vec3_t start, vec3_t dir, int fleshEntityNum, int duration, int size, int endsize,
						  int randorg, int team)
{
	cparticle_t    *p;
	int             i;
	...
	if(i == 0)
	{
		if(team == TEAM_RED)
		{
			p->pshader = cgs.media.smallrFire1;
		}
		else if(team == TEAM_BLUE)
		{
			p->pshader = cgs.media.smallbFire1;
		}
		else
		{
			p->pshader = cgs.media.smallFire1;
	...
	else if(i == 1)
	{
		if(team == TEAM_RED)
		{
			p->pshader = cgs.media.smallrFire2;
		}
		else if(team == TEAM_BLUE)
		{
			p->pshader = cgs.media.smallbFire2;
		}
		else
		{
			p->pshader = cgs.media.smallFire2;
	...
	else if(i == 2)
	{
		if(team == TEAM_RED)
		{
			p->pshader = cgs.media.smallrFire3;
		}
		else if(team == TEAM_BLUE)
		{
			p->pshader = cgs.media.smallbFire3;
		}
		else
		{
			p->pshader = cgs.media.smallFire3;
	...
	else if(i == 3)
	{
		if(team == TEAM_RED)
		{
			p->pshader = cgs.media.smallrFire4;
		}
		else if(team == TEAM_BLUE)
		{
			p->pshader = cgs.media.smallbFire4;
		}
		else
		{
			p->pshader = cgs.media.smallFire4;
	...
	else if(i == 4)
	{
		if(team == TEAM_RED)
		{
			p->pshader = cgs.media.smallrFire5;
		}
		else if(team == TEAM_BLUE)
		{
			p->pshader = cgs.media.smallbFire5;
		}
		else
		{
			p->pshader = cgs.media.smallFire5;
	...
	else if(i == 5)
	{
		if(team == TEAM_RED)
		{
			p->pshader = cgs.media.smallrFire6;
		}
		else if(team == TEAM_BLUE)
		{
			p->pshader = cgs.media.smallbFire6;
		}
		else
		{
			p->pshader = cgs.media.smallFire6;
	...
	else if(i == 6)
	{
		if(team == TEAM_RED)
		{
			p->pshader = cgs.media.smallrFire7;
		}
		else if(team == TEAM_BLUE)
		{
			p->pshader = cgs.media.smallbFire7;
		}
		else
		{
			p->pshader = cgs.media.smallFire7;
	...
	else if(i == 7)
	{
		if(team == TEAM_RED)
		{
			p->pshader = cgs.media.smallrFire8;
		}
		else if(team == TEAM_BLUE)
		{
			p->pshader = cgs.media.smallbFire8;
		}
		else
		{
			p->pshader = cgs.media.smallFire8;
	...
	else if(i == 8)
	{
		if(team == TEAM_RED)
		{
			p->pshader = cgs.media.smallrFire9;
		}
		else if(team == TEAM_BLUE)
		{
			p->pshader = cgs.media.smallbFire9;
		}
		else
		{
			p->pshader = cgs.media.smallFire9;
	...
	else if(i == 9)
	{
		if(team == TEAM_RED)
		{
			p->pshader = cgs.media.smallrFire10;
		}
		else if(team == TEAM_BLUE)
		{
			p->pshader = cgs.media.smallbFire10;
		}
		else
		{
			p->pshader = cgs.media.smallFire10;
	...
	else if(i == 10)
	{
		if(team == TEAM_RED)
		{
			p->pshader = cgs.media.smallrFire11;
		}
		else if(team == TEAM_BLUE)
		{
			p->pshader = cgs.media.smallbFire11;
		}
		else
		{
			p->pshader = cgs.media.smallFire11;
	...
	else if(i == 11)
	{
		if(team == TEAM_RED)
		{
			p->pshader = cgs.media.smallrFire12;
		}
		else if(team == TEAM_BLUE)
		{
			p->pshader = cgs.media.smallbFire12;
		}
		else
		{
			p->pshader = cgs.media.smallFire12;
	...
	else if(i == 12)
	{
		if(team == TEAM_RED)
		{
			p->pshader = cgs.media.smallrFire13;
		}
		else if(team == TEAM_BLUE)
		{
			p->pshader = cgs.media.smallbFire13;
		}
		else
		{
			p->pshader = cgs.media.smallFire13;
	...
	else if(i == 13)
	{
		if(team == TEAM_RED)
		{
			p->pshader = cgs.media.smallrFire14;
		}
		else if(team == TEAM_BLUE)
		{
			p->pshader = cgs.media.smallbFire14;
		}
		else
		{
			p->pshader = cgs.media.smallFire14;
	...
	else if(i == 14)
	{
		if(team == TEAM_RED)
		{
			p->pshader = cgs.media.smallrFire15;
		}
		else if(team == TEAM_BLUE)
		{
			p->pshader = cgs.media.smallbFire15;
		}
		else
		{
			p->pshader = cgs.media.smallFire15;
	...
	else if(i == 15)
	{
		if(team == TEAM_RED)
		{
			p->pshader = cgs.media.smallrFire16;
		}
		else if(team == TEAM_BLUE)
		{
			p->pshader = cgs.media.smallbFire16;
		}
		else
		{
			p->pshader = cgs.media.smallFire16;
	...
	else if(i == 16)
	{
		if(team == TEAM_RED)
		{
			p->pshader = cgs.media.smallrFire17;
		}
		else if(team == TEAM_BLUE)
		{
			p->pshader = cgs.media.smallbFire17;
		}
		else
		{
			p->pshader = cgs.media.smallFire17;
...
void CG_Particle_FireSparks(qhandle_t pshader, vec3_t start, vec3_t dir, int fleshEntityNum, int duration, int size, int endsize,
							int randorg)
{
	cparticle_t    *p;
...
void CG_Particle_HalfBurstSparks(qhandle_t pshader, vec3_t start, vec3_t dir, int team)
{
	...
	p->endtime = cg.time + 430;

	p->startfade = cg.time + (p->endtime / 2);
	p->type = P_SMOKE;
	...
	VectorCopy(start, p->org);
	VectorNormalize2(pdir, ppdir);
	p->vel[0] = (ppdir[0] + pdir[0] / (109.3f + random())) + (crandom() * 40);
	p->vel[1] = (ppdir[1] + pdir[1] / (109.3f + random())) + (crandom() * 40);
	p->vel[2] = (ppdir[2] + pdir[2] / (109.3f + random())) + (crandom() * 40);
	VectorMA(p->vel, 40, dir, p->vel);
	VectorClear(p->accel);

	VectorCopy(p->vel, p->accel);
...
void CG_Particle_Full360BurstSparks(qhandle_t pshader, vec3_t start, vec3_t dir)
{
	...
	p->vel[0] = (crandom() * 120) / 1.5f;
	p->vel[1] = (crandom() * 120) / 1.5f;
	p->vel[2] = (crandom() * 120) / 1.5f;

	p->accel[0] = crandom() * 0.9f;
	p->accel[1] = crandom() * 0.9f;
	p->accel[2] = crandom() * 0.9f;
...
void CG_Particle_Full360BurstBlood(qhandle_t pshader, vec3_t start, vec3_t dir)
{
	...
	p->endtime = cg.time + 900;
	p->startfade = cg.time + (900 / 2);
	p->type = P_SMOKE;

	p->width = 6;
	p->height = 6;
	p->endheight = 17 + rand() % 10;
	p->endwidth = 17 + rand() % 10;

	VectorCopy(start, p->org);
	VectorClear(p->accel);

	p->vel[0] = (crandom() * 80) / 1.5f;
	p->vel[1] = (crandom() * 80) / 1.5f;
	p->vel[2] = (crandom() * 80) / 1.5f;

	p->accel[0] = crandom() * 1.1f;
	p->accel[1] = crandom() * 1.1f;
	p->accel[2] = crandom() * 1.1f;
...
void CG_Particle_FireSmoke(qhandle_t pshader, vec3_t start, vec3_t dir, int fleshEntityNum, int duration, int size, int endsize,
						   int randorg)
{
	cparticle_t    *p;
	...
	p->startfade = cg.time + (duration / 4);
	p->type = P_SMOKE;
...
void CG_ParticleBubbles(qhandle_t pshader, vec3_t origin, float turb, float randsize)
{
	...
	p->endtime = p->time + 1000 + random() * 250;
	p->startfade = p->time + 800;

	p->type = P_SMOKE;

	size = turb + (crandom() * randsize);
...
void CG_ParticleBloodCloud(centity_t * cent, vec3_t origin, vec3_t dir)
{
	...
	for(i = 0; i < dist; i++)
	{
		...
		p->endtime = cg.time + 900 + (crandom() * 100);
		p->startfade = cg.time;
		p->width = LARGESIZE;
		p->height = LARGESIZE;

		p->endheight = LARGESIZE + 90;
		p->endwidth = LARGESIZE + 90;
		p->type = P_SMOKE;
									  ...
									  float width, float height, float alpha, char *shadername)
{
	...
	p->width = height;
	p->endheight = 0;
	p->endwidth = p->width * 1.5f;

	p->rotate = 0;

	p->type = P_SPLASH;

	p->pshader = trap_R_RegisterShader(shadername);
