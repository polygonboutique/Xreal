...
static void CG_DrawClientScore( int y, score_t *score, float *color, float fade, qboolean largeFormat ) {
	...
	CH_DrawStringExt( SB_SCORELINE_XB + (SB_RATING_WIDTH / 2)+20, y+2, string, TINYCHAR_WIDTH+3, TINYCHAR_HEIGHT+3, colorWhite, 64, qfalse);
...
qboolean CG_DrawOldScoreboard( void ) {
	...
	// fragged by ... line
	if ( cg.killerName && cg.predictedPlayerState.pm_type != PM_INTERMISSION
...
/*	st = va("^3%2i:%s%i:%s%i %s  %s %s %i, %i",
		...
		Com_sprintf(st, sizeof(st),
			"^3%2i:%s%i:%s%i %s  %s %s %i, %i",(1 + (tm.tm_hour+11) % 12),		// 12 hour format
				(tm.tm_min > 9 ? "" : "0"),		// minute padding
				tm.tm_min,
				(tm.tm_sec > 9 ? "" : "0"),		// second padding
				tm.tm_sec,
