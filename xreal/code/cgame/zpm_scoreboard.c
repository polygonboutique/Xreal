...
static void CG_DrawClientZPMScoreR( int y, score_t *score, float *color, float fade, qboolean largeFormat ) {
	...
	if ( score->client == cg.snap->ps.clientNum ) {
		...
		hcolor[3] = fade * 0.7;
		CG_FillRect( SB_SCORELINE_XR +10, y, 
			640, BIGCHAR_HEIGHT+1, hcolor );
	}
	...
	CH_DrawStringExt( SB_SCORELINE_XR + (SB_RATING_WIDTH / 2)-50+15, y+2, string, TINYCHAR_WIDTH+3, TINYCHAR_HEIGHT+3, colorWhite, 64, qfalse);
...
static void CG_DrawClientZPMScoreB( int y, score_t *score, float *color, float fade, qboolean largeFormat ) {
	...
	CH_DrawStringExt( SB_SCORELINE_XB + (SB_RATING_WIDTH / 2)-50+15, y+2, string, TINYCHAR_WIDTH+3, TINYCHAR_HEIGHT+3, colorWhite, 64, qfalse);
...
*/
	...
	st = va("^3%2i:%s%i:%s%i %s  %s %s %i, %i",
				...
				tm.tm_sec,
				(tm.tm_hour < 12 ? "am" : "pm"),weekStr[tm.tm_wday],monthStr[tm.tm_mon],
				tm.tm_mday,
				1900 + tm.tm_year );
