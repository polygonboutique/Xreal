...
void CG_DrawMultiFontStringExt( int x, int y, const char *string, const float *setColor, 
		qboolean forceColor, qboolean shadow, int charWidth, int charHeight, int maxChars, qhandle_t ChShader ) {
	vec4_t		color;
	const char	*s;
	int			xx;
	int			cnt;
...
//	char	p[1024];
	...
	ChShader2 = cgs.media.charsetShader;
...
void CG_TileClear( void ) {
	...
	if ( cg.refdef[0].x == 0 && cg.refdef[0].y == 0 && 
		cg.refdef[0].width == w && cg.refdef[0].height == h ) {
