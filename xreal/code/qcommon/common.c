...
Com_InitZoneMemory (void)
{
  ...
  cv =
    Cvar_Get ("com_zoneMegs", DEF_COMZONEMEGS_S, CVAR_LATCH | CVAR_ARCHIVE);
...
Com_EventLoop (void)
{
  ...
  while (1)
	...
	case SE_PACKET:
	  ...
	  Com_Memcpy (buf.data, (byte *) ((netadr_t *) ev.evPtr + 1),
		      buf.cursize);
	  if (com_sv_running->integer)
	    {
	      Com_RunAndTimeServerPacket (&evFrom, &buf);
...
static void
Com_PrintMatrix (const matrix_t m)
{
  Com_Printf ("%i %i %i %i\n"
	      "%i %i %i %i\n"
	      "%i %i %i %i\n"
	      ...
	      (int) m[8], (int) m[9], (int) m[10], (int) m[11], (int) m[12],
	      (int) m[13], (int) m[14], (int) m[15]);
}

static void
Com_PrintQuat (const quat_t q)
{
  Com_Printf ("%f %f %f %f\n", q[0], q[1], q[2], q[3]);
}
...
Com_GenerateMediaTXT_f (void)
{
  ...
  while (qtrue)
	...
	break;
      ...
      // bleh german shell
      if (!Q_stricmp (token, "properties")
	  || !Q_stricmp (token, "eigenschaften"))
	{
	  // dump last entry
	  ...
	  if (Q_stricmp (mediaName + len - 4, ".txt") != 0 &&
	      ...
	      if (firstEntry)
		{
		  ...
		  len = 0;
		  while (len <
			 (maxMediaLen + maxCopyrightLen + maxLicenseLen + 6))
		    {
		      FS_Printf (f, "=");
		      len++;
		    }
...
Com_GenerateCorePK3_f (void)
{
  ...
  Com_RealTime (&dt);
  Q_strncpyz (pakName,
	      va ("base/core-%04d%02d%02d.pk3", 1900 + dt.tm_year,
		  dt.tm_mon + 1, dt.tm_mday), sizeof (pakName));
...
Com_DetectSSE (void)
{
  ...
  if (!(Sys_GetProcessorFeatures () & CF_SSE))
    {
      Com_Error (ERR_FATAL,
		 "Binaries were compiled with SSE optimizations but your CPU doesn't support SSE");
    }
#endif
}
  ...
  if (com_showtrace->integer)
    {
      ...
      Com_Printf ("%4i traces  (%ib %ip %it) %4i points\n", c_traces,
		  c_brush_traces, c_patch_traces, c_trisoup_traces,
		  c_pointcontents);
      c_traces = 0;
      c_brush_traces = 0;
      c_patch_traces = 0;
...
*/
static void
Field_CompleteFilename (const char *dir, const char *ext, qboolean stripExt)
{
  matchCount = 0;
  shortestMatch[0] = 0;
