...
int AAS_BestReachableFromJumpPadArea(vec3_t origin, vec3_t mins, vec3_t maxs)
{
	...
	for(ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		AAS_ClientMovementHitBBox(&move, -1, areastart, PRESENCE_NORMAL,
								  qfalse, velocity, cmdmove, 0, 30, 0.1f, bboxmins, bboxmaxs, bot_visualizejumppads);
		if(move.frames < 30)
		{
			bestareanum = 0;
			bestareavolume = 0;
		...
		if(aasworld.areasettings[AAS_PointAreaNum(testpoint)].areaflags & AREA_LIQUID)
		{
			...
			if(water_bestdist < aassettings.phys_maxwaterjump + 24)
			{
				...
				if((aasworld.areasettings[area1num].
					presencetype & PRESENCE_NORMAL) && (aasworld.areasettings[area2num].presencetype & PRESENCE_NORMAL))
				{
		...
		for(j = 0; j < area2->numfaces; j++)
		{
			...
			for(k = 0; k < face1->numedges; k++)
			{
				...
				for(l = 0; l < face2->numedges; l++)
				{
					...
					//
					bestdist =
						AAS_ClosestEdgePoints(v1, v2, v3, v4, plane1, plane2, beststart, bestend, beststart2, bestend2, bestdist);
				}
			}
		}
	}
...
//      Log_Write("shortest distance between %d and %d is %f\r\n", area1num, area2num, bestdist);
		...
		for(i = 0; i < 3; i++)
		{
			...
			AAS_PredictClientMovement(&move, -1, beststart, PRESENCE_NORMAL,
									  qtrue, velocity, cmdmove, 3, 30, 0.1f, stopevent, 0, qfalse);
		...
		if(velocity[0] || velocity[1])
		{
			...
			for(i = 0; i < 20; i++)
			{
				AAS_PredictClientMovement(&move, -1, areastart,
										  ...
										  SE_HITGROUNDDAMAGE | SE_TOUCHJUMPPAD | SE_TOUCHTELEPORTER, 0, bot_visualizejumppads);
			...
			if(ret && speed < 300)
			{
				...
				//if (hordist < 1.6 * (facecenter[2] - areastart[2]))
				{
					...
					if(move.frames < 30 &&
					   !(move.
						 stopevent & (SE_ENTERSLIME | SE_ENTERLAVA |
									  SE_HITGROUNDDAMAGE)) && (move.stopevent & (SE_HITGROUNDAREA | SE_TOUCHJUMPPAD)))
					{
						//create a rocket or bfg jump reachability from area1 to area2
						lreach = AAS_AllocReachability();
						if(!lreach)
