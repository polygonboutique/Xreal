...
void AAS_CreateReversedReachability(void)
{
	...
	ptr =
		(char *)GetClearedMemory(aasworld.numareas *
								 sizeof(aas_reversedreachability_t) + aasworld.reachabilitysize * sizeof(aas_reversedlink_t));
	//
...
aas_routingcache_t *AAS_ReadCache(fileHandle_t fp)
{
	...
	cache->reachabilities =
		...
		sizeof(unsigned short) + (size - sizeof(aas_routingcache_t) + sizeof(unsigned short)) / 3 * 2;
	return cache;
		...
		for(i = 0, revlink = revreach->first; revlink; revlink = revlink->next, i++)
		{
			...
			if(!areacache->traveltimes[clusterareanum] || areacache->traveltimes[clusterareanum] > t)
			{
				...
				nextupdate->areatraveltimes =
					aasworld.areatraveltimes[nextareanum][linknum - aasworld.areasettings[nextareanum].firstreachablearea];
...
//===========================================================================
int
AAS_PredictRoute(struct aas_predictroute_s *route, int areanum,
