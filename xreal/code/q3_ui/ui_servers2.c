...
static char quake3worldMessage[] =
  "Visit http://xreal.sourceforge.net - News, Events, Files";
...
ArenaServers_Remove (void)
{
  ...
  for (i = 0; i < g_arenaservers.numfavoriteaddresses; i++)
    {
      if (!Q_stricmp
	  (g_arenaservers.favoriteaddresses[i], servernodeptr->adrstr))
	{
	  ...
	  if (i < g_arenaservers.numfavoriteaddresses - 1)
	    {
	      ...
	      memcpy (&g_arenaservers.favoriteaddresses[i],
		      ...
		      (g_arenaservers.numfavoriteaddresses - i -
		       1) * MAX_ADDRESSLENGTH);
	    }
	  g_arenaservers.numfavoriteaddresses--;
	  memset (&g_arenaservers.
		  favoriteaddresses[g_arenaservers.numfavoriteaddresses], 0,
		  MAX_ADDRESSLENGTH);
	  break;
	}
    }
...
ArenaServers_Insert (char *adrstr, char *info, int pingtime)
{
  ...
  if (*g_arenaservers.numservers >= g_arenaservers.maxservers)
  ...
  else
    {
      // next slot
      servernodeptr =
	g_arenaservers.serverlist + (*g_arenaservers.numservers);
  ...
  if (*s)
    {
      ...
      Q_strncpyz (servernodeptr->gamename, s,
...
ArenaServers_InsertFavorites (void)
{
  ...
  for (i = 0; i < g_arenaservers.numfavoriteaddresses; i++)
	...
	if (!Q_stricmp
	    (g_arenaservers.favoriteaddresses[i],
...
ArenaServers_LoadFavorites (void)
{
  ...
  for (i = 0; i < MAX_FAVORITESERVERS; i++)
	...
	{
	  // add new server
	  Q_strncpyz (g_favoriteserverlist[g_numfavoriteservers].adrstr,
		      adrstr, MAX_ADDRESSLENGTH);
...
ArenaServers_DoRefresh (void)
{
  ...
  if (g_servertype == UIAS_FAVORITES)
    ...
    {
      g_arenaservers.numqueriedservers =
	trap_LAN_GetServerCount (g_servertype);
    }
  ...
  for (i = 0;
       i < MAX_PINGREQUESTS
       && g_arenaservers.currentping < g_arenaservers.numqueriedservers; i++)
    {
	...
	{
	  strcpy (adrstr,
		  g_arenaservers.favoriteaddresses[g_arenaservers.
						   currentping]);
	}
      else
	{
