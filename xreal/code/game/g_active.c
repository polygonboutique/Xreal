...
void G_TouchTriggers(gentity_t * ent)
{
	...
	for(i = 0; i < num; i++)
	{
		...
		if(!(hit->flags & FL_DROPPED_ITEM) &&
		   ...
		   ent->client->sess.sessionTeam == TEAM_BLUE && ent->client->ps.powerups[PW_BLUEFLAG])
		{
		}
		else if(!(hit->flags & FL_DROPPED_ITEM) &&
				strcmp(hit->classname, "team_CTF_redflag") == 0 &&
				ent->client->sess.sessionTeam == TEAM_RED && ent->client->ps.powerups[PW_REDFLAG])
		{
		}
...
//unlagged - true ping
	...
	if(client->sess.speconly == 1 && client->sess.sessionTeam != TEAM_SPECTATOR)
	{
		SetTeam(ent, "s");
	}
