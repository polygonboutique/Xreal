		...
		if (ent->health > 0
			...
			if ( envirosuit || spawnprotect) {
				...
				if (ent->watertype & CONTENTS_LAVA) {
					G_Damage (ent, NULL, NULL, NULL, NULL, 
						8*waterlevel, 0, MOD_LAVA);
				}

				if (ent->watertype & CONTENTS_SLIME) {
					G_Damage (ent, NULL, NULL, NULL, NULL, 
						4*waterlevel, 0, MOD_SLIME);
				}
			}
		}
	}
...
void ClientTimerActions( gentity_t *ent, int msec ) {
	...
	if( bg_itemlist[client->ps.stats[STAT_PERSISTANT_POWERUP]].giTag == PW_AMMOREGEN ) {
		int w, max, inc, t, i;
    int weapList[]={WP_MACHINEGUN,WP_SHOTGUN,WP_GRENADE_LAUNCHER,WP_ROCKET_LAUNCHER,WP_LIGHTNING,WP_RAILGUN,WP_IRAILGUN,WP_PLASMAGUN,WP_BFG,WP_NAILGUN,WP_PROX_LAUNCHER,WP_CHAINGUN};
    ...
    for (i = 0; i < weapCount; i++) {
		  ...
		  switch(w) {
			  ...
			  case WP_IRAILGUN: max = 10; inc = 1; t = 1750; break;
