	...
	if (dist < 80){
		if (bs->inventory[INVENTORY_GAUNTLET] > 0){
			...
			if (bs->inventory[INVENTORY_LIGHTNING] > 0
				&& bs->inventory[INVENTORY_LIGHTNINGAMMO] > 1){
				weapon = WP_LIGHTNING;
			} else
			if (bs->inventory[INVENTORY_SHOTGUN] > 0
				&& bs->inventory[INVENTORY_SHELLS] > 1){
				weapon = WP_SHOTGUN;
			} else 
			if (bs->inventory[INVENTORY_FLAMETHROWER] > 0
				&& bs->inventory[INVENTORY_FUEL] > 1){
				weapon = WP_FLAMETHROWER;
			} else
			if (bs->inventory[INVENTORY_PLASMAGUN] > 0
				&& bs->inventory[INVENTORY_CELLS] > 1){
				weapon = WP_PLASMAGUN;
			} else
			if (bs->inventory[INVENTORY_GRENADELAUNCHER] > 0
				&& bs->inventory[INVENTORY_GRENADES] > 1){
			...
			if (bs->inventory[INVENTORY_IRAILGUN] > 0
				&& bs->inventory[INVENTORY_ISLUGS] > 1){
				weapon = WP_IRAILGUN;
			} else
...
float BotAggression(bot_state_t *bs) {
	...
	if (bs->inventory[INVENTORY_GAUNTLET] > 0 ) return 100;
...
int BotCanAndWantsToRocketJump(bot_state_t *bs) {
	...
	if (rocketjumper < 0.2) return qfalse;
	return qtrue;
}
