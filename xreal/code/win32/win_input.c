...
void IN_DIMouse(int *mx, int *my)
{
	DIDEVICEOBJECTDATA od[DI_BUFFER_SIZE];
	DIMOUSESTATE2   state;
	DWORD           dwElements;
	HRESULT         hr;
	int             value;
	...
	for(;;)
	{
		...
		for(i = 0; i < dwElements; i++)
		{
			switch (od[i].dwOfs)
			{
				...
				case DIMOFS_BUTTON3:
				case DIMOFS_BUTTON4:
					Sys_QueEvent(od[i].dwTimeStamp, SE_KEY, K_MOUSE1 + od[i].dwOfs - DIMOFS_BUTTON0, (od[i].dwData & 0x80), 0,
								 NULL);
					break;
				case DIMOFS_BUTTON5:
...
void IN_StartupMouse(void)
{
	...
	if((g_wv.osversion.dwPlatformId == VER_PLATFORM_WIN32_NT) && (g_wv.osversion.dwMajorVersion == 4))
	{
...
void IN_JoyMove(void)
{
	...
	if(in_debugJoystick->integer)
	{
		Com_Printf("%8x %5i %5.2f %5.2f %5.2f %5.2f %6i %6i\n",
				   ...
				   JoyToF(joy.ji.dwZpos), JoyToF(joy.ji.dwRpos), JoyToI(joy.ji.dwUpos), JoyToI(joy.ji.dwVpos));
	}
...
void IN_Init(void)
{
	...
	// make sure these are set unmodified after initialization
