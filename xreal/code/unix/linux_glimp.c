...
static void install_grabs(void)
{
	...
	XGrabPointer(dpy, win,		// bk010108 - do this earlier?
				 False, MOUSE_MASK, GrabModeAsync, GrabModeAsync, win, None, CurrentTime);
...
static void uninstall_grabs(void)
{
	...
	XChangePointerControl(dpy, qtrue, qtrue, mouse_accel_numerator, mouse_accel_denominator, mouse_threshold);
...
static qboolean repeated_press(XEvent * event)
{
	...
	if(X11_PendingInput())
	{
		...
		if((peekevent.type == KeyPress) &&
		   (peekevent.xkey.keycode == event->xkey.keycode) && (peekevent.xkey.time == event->xkey.time))
		{
			repeated = qtrue;
			...
			case MotionNotify:
				...
				if(mouse_active)
				{
					...
					else
					{
						...
						if(event.xmotion.x == glConfig.vidWidth / 2 && event.xmotion.y == glConfig.vidHeight / 2)
						{
							mwx = glConfig.vidWidth / 2;
							mwy = glConfig.vidHeight / 2;
							if(t - mouseResetTime > MOUSE_RESET_DELAY)
	...
	if(Q_stristr(glConfig.extensions_string, "GL_EXT_framebuffer_object"))
	{
		if(r_ext_framebuffer_object->value)
		{
			...
			qglGetFramebufferAttachmentParameterivEXT =
				(PFNGLGETFRAMEBUFFERATTACHMENTPARAMETERIVEXTPROC) qwglGetProcAddress("glGetFramebufferAttachmentParameterivEXT");
...
qboolean GLimp_SpawnRenderThread(void (*function) (void))
{
	...
	ret = pthread_create(&renderThread, NULL,	// attributes
						 GLimp_RenderThreadWrapper, NULL);	// argument
	if(ret)
	{
