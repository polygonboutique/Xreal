...
static const int numSkillLevels =
  sizeof (skillLevels) / sizeof (const char *);
...
static char quake3worldMessage[] =
  "Visit www.quake3world.com - News, Community, Events, Files";
#endif
...
int ProcessNewUI (int command, int arg0, int arg1, int arg2, int arg3,
...
AssetCache ()
{
  ...
  uiInfo.newHighScoreSound =
    trap_S_RegisterSound ("sound/feedback/newhighscore.wav", qfalse);
}
...
void
Text_PaintChar (float x, float y, float width, float height, float scale,
		      ...
		      int style)
{
  ...
  if (text)
      ...
      if (cursorPos == len && !((uiInfo.uiDC.realTime / BLINK_DIVISOR) & 1))
	{
	  ...
	  Text_PaintChar (x, y - yadj,
			  glyph2->imageWidth,
			  glyph2->imageHeight, useScale, glyph2->s, glyph2->t,
...
_UI_Refresh (int realtime)
{
  ...
  if (Menu_Count () > 0)
    {
      UI_DrawHandlePic (uiInfo.uiDC.cursorx - 16, uiInfo.uiDC.cursory - 16,
			32, 32, uiInfo.uiDC.Assets.cursor);
    }
...
UI_LoadMenus (const char *menuFile, qboolean reset)
{
  ...
  if (!handle)
    {
      trap_Error (va
		  (S_COLOR_YELLOW "menu file not found: %s, using default\n",
		   menuFile));
...
static void
UI_DrawNetGameType (rectDef_t * rect, float scale, vec4_t color,
		    int textStyle)
{
...
static void
UI_DrawJoinGameType (rectDef_t * rect, float scale, vec4_t color,
		     int textStyle)
{
      ...
      if (uiInfo.teamList[i].cinematic >= -2)
	{
	  if (uiInfo.teamList[i].cinematic == -1)
		...
		trap_CIN_PlayCinematic (va
					("%s.roq",
					 uiInfo.teamList[i].imageName), 0, 0,
...
UI_DrawPreviewCinematic (rectDef_t * rect, float scale, vec4_t color)
{
  if (uiInfo.previewMovie > -2)
	...
	trap_CIN_PlayCinematic (va
				("%s.roq",
				 uiInfo.movieList[uiInfo.movieIndex]), 0, 0,
				0, 0, (CIN_loop | CIN_silent));
		   ...
		   int num, int textStyle)
{
  ...
  int value =
    trap_Cvar_VariableValue (va
			     (blue ? "ui_blueteam%i" : "ui_redteam%i", num));
  const char *text;
...
UI_DrawEffects (rectDef_t * rect, float scale, vec4_t color)
{
  ...
  UI_DrawHandlePic (rect->x + uiInfo.effectsColor * 16 + 8, rect->y - 16, 16,
		    12, uiInfo.uiDC.Assets.fxPic[uiInfo.effectsColor]);
}
...
static void
UI_DrawMapTimeToBeat (rectDef_t * rect, float scale, vec4_t color,
		      int textStyle)
{
...
static void
UI_DrawMapCinematic (rectDef_t * rect, float scale, vec4_t color,
		     qboolean net)
{
  ...
  if (uiInfo.mapList[map].cinematic >= -1)
      ...
      if (uiInfo.mapList[map].cinematic == -1)
	{
	  uiInfo.mapList[map].cinematic =
	    trap_CIN_PlayCinematic (va
				    ("%s.roq",
				     uiInfo.mapList[map].mapLoadName), 0, 0,
				    0, 0, (CIN_loop | CIN_silent));
	}
...
UI_DrawNetFilter (rectDef_t * rect, float scale, vec4_t color, int textStyle)
{
  ...
  Text_Paint (rect->x, rect->y, scale, color,
	      va ("Filter: %s",
		  serverFilters[ui_serverFilterType.integer].description), 0,
	      0, textStyle);
}
...
static void
UI_DrawTierMapName (rectDef_t * rect, float scale, vec4_t color,
		    int textStyle)
{
  int i, j;
...
static void
UI_DrawTierGameType (rectDef_t * rect, float scale, vec4_t color,
		     int textStyle)
{
  int i, j;
  ...
  Text_Paint (rect->x, rect->y, scale, color,
	      uiInfo.gameTypes[uiInfo.tierList[i].gameTypes[j]].gameType, 0,
	      0, textStyle);
}
...
static void
UI_DrawOpponentName (rectDef_t * rect, float scale, vec4_t color,
		     int textStyle)
{
  Text_Paint (rect->x, rect->y, scale, color,
...
UI_DrawCrosshair (rectDef_t * rect, float scale, vec4_t color)
{
  ...
  UI_DrawHandlePic (rect->x, rect->y - rect->h, rect->w, rect->h,
		    uiInfo.uiDC.Assets.crosshairShader[uiInfo.
						       currentCrosshair]);
  trap_R_SetColor (NULL);
}
...
static void
UI_DrawSelectedPlayer (rectDef_t * rect, float scale, vec4_t color,
		       int textStyle)
{
  ...
  Text_Paint (rect->x, rect->y, scale, color,
	      (uiInfo.
	       teamLeader) ? UI_Cvar_VariableString ("cg_selectedPlayerName")
  ...
  if (uiInfo.serverStatus.refreshActive)
    {
      ...
      Text_Paint (rect->x, rect->y, scale, newColor,
		  va ("Getting info for %d servers (ESC to cancel)",
		      trap_LAN_GetServerCount (ui_netSource.integer)), 0, 0,
		  textStyle);
    }
  else
    {
		  ...
		  uiInfo.serverStatus.motdLen)
		{
		  uiInfo.serverStatus.motdPaintX +=
		    Text_Width (&uiInfo.serverStatus.
				motd[uiInfo.serverStatus.motdOffset], scale,
				1) - 1;
...
static void
UI_DrawKeyBindStatus (rectDef_t * rect, float scale, vec4_t color,
		      int textStyle)
{
		  ...
		  if (trap_Cvar_VariableValue ("sv_killserver") == 0)
		    {
		      ...
		      trap_S_StartLocalSound (uiInfo.newHighScoreSound,
					      CHAN_ANNOUNCER);
...
UI_ServersQsortCompare (const void *arg1, const void *arg2)
{
  return trap_LAN_CompareServers (ui_netSource.integer,
				  ...
				  uiInfo.serverStatus.sortDir, *(int *) arg1,
				  *(int *) arg2);
}
  ...
  if (String_Parse (args, &name))
      ...
      if (Q_stricmp (name, "StartServer") == 0)
	{
	  ...
	  trap_Cvar_SetValue ("g_gametype",
			      Com_Clamp (0, 8,
					 uiInfo.gameTypes[ui_netGameType.
							  integer].gtEnum));
	  trap_Cvar_Set ("g_redTeam", UI_Cvar_VariableString ("ui_teamName"));
	  trap_Cvar_Set ("g_blueTeam",
			 UI_Cvar_VariableString ("ui_opponentName"));
	  trap_Cmd_ExecuteText (EXEC_APPEND,
				va ("wait ; wait ; map %s\n",
      ...
      else if (Q_stricmp (name, "verifyCDKey") == 0)
	{
	  ...
	  if (trap_VerifyCDKey
	      (buff, UI_Cvar_VariableString ("cdkeychecksum")))
	    {
...
UI_MapCountByGameType (qboolean singlePlayer)
{
  ...
  c = 0;
  game =
    singlePlayer ? uiInfo.gameTypes[ui_gameType.integer].gtEnum : uiInfo.
    gameTypes[ui_netGameType.integer].gtEnum;
...
static qboolean
UI_hasSkinForBase (const char *base)
{
  char test[1024];

  Com_sprintf (test, sizeof (test), "models/players/%s/lower_default.skin",
	       base);
  ...
  Com_sprintf (test, sizeof (test),
	       "models/players/characters/%s/lower_default.skin", base);
  if (trap_FS_FOpenFile (test, 0, FS_READ))
    {
      return qtrue;
    }
...
UI_BinaryServerInsertion (int num)
{
  ...
  while (mid > 0)
	...
	trap_LAN_CompareServers (ui_netSource.integer,
				 ...
				 uiInfo.serverStatus.displayServers[offset +
								    mid]);
      // if equal
      if (res == 0)
	{
	  ...
	  while (p && *p)
		...
		*p++ = '\0';
		...
		break;
	      *p++ = '\0';
	      name = p;
	      Com_sprintf (&info->pings[len], sizeof (info->pings) - len,
			   "%d", i);
			  ...
			  MAX_FOUNDPLAYER_SERVERS - 1)
			{
			  ...
			  Q_strncpyz (uiInfo.
				      foundPlayerServerAddresses[uiInfo.
								 ...
								 - 1],
				      uiInfo.pendingServerStatus.server[i].
				      adrstr,
				      sizeof (uiInfo.
  ...
  if (feederID == FEEDER_HEADS)
      ...
      if (index >= 0 && index < uiInfo.characterCount)
	{
	  if (uiInfo.characterList[index].headImage == -1)
		...
		trap_R_RegisterShaderNoMip (uiInfo.characterList[index].
					    imageName);
	    }
...
UI_FeederSelection (float feederID, int index)
{
  ...
  if (feederID == FEEDER_HEADS)
	...
	{
	  trap_Cvar_Set ("model",
			 va ("%s", uiInfo.characterList[index].base));
  ...
  else if (feederID == FEEDER_MAPS || feederID == FEEDER_ALLMAPS)
    {
      ...
      map =
	(feederID ==
	 FEEDER_ALLMAPS) ? ui_currentNetMap.integer : ui_currentMap.integer;
      if (uiInfo.mapList[map].cinematic >= 0)
	{
  ...
  while (1)
      ...
      if (token[0] == '{')
	{
	  ...
	  uiInfo.teamList[uiInfo.teamCount].teamIcon =
	    trap_R_RegisterShaderNoMip (uiInfo.teamList[uiInfo.teamCount].
					imageName);
	  uiInfo.teamList[uiInfo.teamCount].teamIcon_Metal =
	    trap_R_RegisterShaderNoMip (va
					("%s_metal",
					 uiInfo.teamList[uiInfo.teamCount].
					 imageName));
	  ...
	  for (i = 0; i < TEAM_MEMBERS; i++)
	    {
	      ...
	      if (!String_Parse
		  (p, &uiInfo.teamList[uiInfo.teamCount].teamMembers[i]))
		{
		  return qfalse;
		}
	    }
...
Character_Parse (char **p)
{
  ...
  while (1)
      ...
      if (token[0] == '{')
	{
	  ...
	  if (!String_Parse
	      (p, &uiInfo.characterList[uiInfo.characterCount].name)
	      || !String_Parse (p, &tempStr))
	    {
	      return qfalse;
	    }
	  ...
	  if (tempStr && (!Q_stricmp (tempStr, "female")))
		...
		String_Alloc (va ("Janet"));
	    }
	  else if (tempStr && (!Q_stricmp (tempStr, "male")))
	    {
	      uiInfo.characterList[uiInfo.characterCount].base =
		String_Alloc (va ("Visor"));
	    }
	  else
	    {
	      uiInfo.characterList[uiInfo.characterCount].base =
      ...
      if (token[0] == '{')
	{
	  ...
	  if (join)
	  ...
	  else
	    {
		  ...
		  || !Int_Parse (p,
				 &uiInfo.gameTypes[uiInfo.numGameTypes].
				 gtEnum))
		{
		  return qfalse;
		}
...
MapList_Parse (char **p)
{
  ...
  while (1)
      ...
      if (token[0] == '{')
	{
	  ...
	  if (!String_Parse
	      (p, &uiInfo.mapList[uiInfo.mapCount].opponentName))
	    {
	      return qfalse;
	    }
	  ...
	  while (1)
	      ...
	      if (token[0] >= '0' && token[0] <= '9')
		{
		  ...
		  if (!Int_Parse
		      (p,
		       &uiInfo.mapList[uiInfo.mapCount].timeToBeat[token[0] -
								   0x30]))
		    {
		      return qfalse;
		    }
	  ...
	  uiInfo.mapList[uiInfo.mapCount].levelShot =
	    trap_R_RegisterShaderNoMip (va
					("levelshots/%s_small",
					 uiInfo.mapList[uiInfo.mapCount].
					 mapLoadName));
  ...
  if (handle >= 0)
      ...
      if (handle == UI_MAPCINEMATIC)
	{
	  if (uiInfo.mapList[ui_currentMap.integer].cinematic >= 0)
	    {
	      trap_CIN_StopCinematic (uiInfo.mapList[ui_currentMap.integer].
				      cinematic);
	...
	{
	  int i =
	    UI_TeamIndexFromName (UI_Cvar_VariableString ("ui_teamName"));
  ...
  if (value > 1024 * 1024 * 1024)
    {				// gigs
      ...
      Com_sprintf (buf + strlen (buf), bufsize - strlen (buf), ".%02d GB",
		   (value % (1024 * 1024 * 1024)) * 100 / (1024 * 1024 *
							   1024));
    }
...
void
Text_PaintCenter (float x, float y, float scale, vec4_t color,
...
static void
UI_DisplayDownloadInfo (const char *downloadName, float centerPoint,
  ...
  if (ui_netSource.integer == AS_GLOBAL)
      ...
      if (strlen (ptr))
	{
	  trap_Cmd_ExecuteText (EXEC_NOW,
				va ("globalservers %s full empty\n", ptr));
	}
      else
	{
	  trap_Cmd_ExecuteText (EXEC_NOW,
				va ("globalservers %d full empty\n",
				    (int)
				    trap_Cvar_VariableValue ("protocol")));
	}
    }
}
