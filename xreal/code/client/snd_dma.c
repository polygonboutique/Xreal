...
#define		SOUND_MAX_DIST		1250
#define 	SOUND_ATTENUATE 	(1.0f / SOUND_MAX_DIST);
...
//  Com_Printf("playing %s\n", sfx->soundName);
  ...
  if (entityNum != ENTITYNUM_WORLD)
	...
	{
	  if ((ch->entnum == entityNum) && (ch->allocTime == time)
	      && (ch->thesfx == sfx))
	    {
...
S_AddLoopSounds (void)
{
  ...
  for (i = 0; i < MAX_GENTITIES; i++)
      ...
      if (!loop->active || loop->mergeFrame == loopFrame)
	{
      ...
      S_SpatializeOrigin (loop->origin, AMBIENT_VOLUME, &left_total,
			  &right_total);
...
*/
void
S_Base_RawSamples (int samples, int rate, int width, int s_channels,
