...
CL_WalkDemoExt (char *arg, char *name, int *demofile)
{
  ...
  *demofile = 0;
  while (demo_protocols[i])
    {
      Com_sprintf (name, MAX_OSPATH, "demos/%s.dm_%d", arg,
		   demo_protocols[i]);
  ...
  while (buffptr + 1 < buffend)
      ...
      if (family == NA_IP)
	{
	  if (buffend - buffptr <
	      ...
	      sizeof (addresses[numservers].port) + sizeof ("\\EOT") - 1)
	    break;
...
CL_Frame (int msec)
{
  ...
  if (CL_VideoRecording () && cl_aviFrameRate->integer && msec)
      ...
      if (cls.state == CA_ACTIVE || cl_forceavidemo->integer)
	{
	  ...
	  msec =
	    (int) ceil ((1000.0f / cl_aviFrameRate->value) *
			com_timescale->value);
	  if (msec == 0)
	    {
	      msec = 1;
...
CL_Init (void)
{
  ...
  cl_platformSensitivity =
    Cvar_Get ("cl_platformSensitivity", "1.0", CVAR_ROM);
...
CL_GlobalServers_f (void)
{
  ...
  if ((count = Cmd_Argc ()) < 3 || (masterNum = atoi (Cmd_Argv (1))) < 0
    ...
    {
      Com_Printf
	("usage: globalservers <master# 0-4> <protocol> [keywords]\n");
      return;
    }
