...
*/
static void
S_AL_BufferLoad (sfxHandle_t sfx)
{
  ALenum error;
  ...
  if (info.size == 0)
    {
      ...
      alBufferData (knownSfx[sfx].buffer, AL_FORMAT_MONO16,
		    (void *) dummyData, 2, 22050);
    }
  else
...
S_AL_RegisterSound (const char *sample, qboolean compressed)
{
  ...
  if (s_alPrecache->integer && (!knownSfx[sfx].inMemory)
	       ...
	       int entity, int channel, qboolean local)
{
  ...
  alSourcef (curSource->alSource, AL_REFERENCE_DISTANCE,
...
S_AL_StartSound (vec3_t origin, int entnum, int entchannel, sfxHandle_t sfx)
{
  ...
  if (origin == NULL)
	...
	{
	  S_AL_SrcSetup (src, sfx, SRCPRI_ONESHOT, entnum, entchannel,
			 qfalse);
...
*/
static void
S_AL_SrcLoop (alSrcPriority_t priority, sfxHandle_t sfx, const vec3_t origin,
	      const vec3_t velocity, int entityNum)
{
  int src;
  ...
  if (S_AL_HearingThroughEntity (entityNum))
  ...
  else
    {
      ...
      alSourcefv (curSource->alSource, AL_POSITION, (ALfloat *) sent->origin);
			  ...
			  const vec3_t velocity, sfxHandle_t sfx)
{
  ...
  if (DistanceSquared (sanOrigin, lastListenerOrigin) >
...
S_AL_SrcUpdate (void)
{
  ...
  for (i = 0; i < srcCount; i++)
	...
	alSourcef (curSource->alSource, AL_ROLLOFF_FACTOR,
		   s_alRolloff->value);
      ...
      if (curSource->isLooping)
	{
	  ...
	  if (sent->loopAddedThisFrame)
	      ...
	      if (sent->startLoopingSound)
		{
	      ...
	      if (curSource->local)
		{
		  alSourcei (curSource->alSource, AL_SOURCE_RELATIVE,
			     AL_TRUE);
		  alSourcef (curSource->alSource, AL_ROLLOFF_FACTOR, 0.0f);
		}
	      else
	...
	{
	  alSourcefv (curSource->alSource, AL_POSITION,
...
*/
static void
S_AL_AllocateStreamChannel (void)
{
...
*/
static void
S_AL_RawSamples (int samples, int rate, int width, int channels,
		 const byte * data, float volume)
{
  ...
  alBufferData (buffer, format, (ALvoid *) data, (samples * width * channels),
		rate);
  ...
  // Volume
  alSourcef (streamSource, AL_GAIN,
...
S_AL_Init (soundInterface_t * si)
{
  ...
  if ((enumsupport = alcIsExtensionPresent (NULL, "ALC_ENUMERATION_EXT")))
	...
	s_alDevice =
	  Cvar_Get ("s_alDevice", ALDEVICE_DEFAULT,
		    CVAR_ARCHIVE | CVAR_LATCH);
      else
#endif
	s_alDevice =
	  Cvar_Get ("s_alDevice", defaultdevice, CVAR_ARCHIVE | CVAR_LATCH);
  ...
  if (enumsupport)
    Cvar_Set ("s_alDevice", alcGetString (alDevice, ALC_DEVICE_SPECIFIER));
