...
static void PM_GroundClimbTrace(void) {
	...
	for (i = 0; i <= 4; i++) {
		...
		// if we hit something
		if (trace.fraction < 1.0f && (trace.surfaceFlags & SURF_WALLWALK) && !(trace.surfaceFlags &(SURF_SKY|SURF_SLICK)) && !(trace.entityNum != ENTITYNUM_WORLD && i != 4)) {
			if (i == 2 || i == 3) {
				if (i == 2) {
			...
			if (!VectorCompare(trace.plane.normal, surfNormal)) {
				...
				if (!VectorCompare(trace.plane.normal, refNormal) && !VectorCompare(surfNormal, refNormal) && !VectorCompare(trace.plane.normal, ceilingNormal) && !VectorCompare(surfNormal, ceilingNormal)) {
...
void PmoveSingle(pmove_t *pmove) {
	...
	if (pm->fixedPmove) {
		...
		// or so of network bandwidth, and add some mostly framerate- independent error to make up for the lack of rounding error
		...
		if (VectorLengthSquared(pm->ps->velocity) < 0.25f) {
			...
			// we can stand a little bit of rounding error for the sake of lower bandwidth
