...
static void RailSphereDamage(gentity_t *self) {
	...
	if (self->count >= RAILGUN_SHOCKWAVE_STARTTIME) {
		...
		RailSphereShockWave(self->s.pos.trBase, self->activator, 25, 200, (int)(float)t * RAILGUN_SHOCKWAVE_MAXRADIUS / (RAILGUN_SHOCKWAVE_ENDTIME - RAILGUN_SHOCKWAVE_STARTTIME));
	}

	if (self->count >= RAILGUN_EXPLODE_STARTTIME) {
		// do our damage
		t = self->count - RAILGUN_EXPLODE_STARTTIME;
		RailSphereRadiusDamage(self->s.pos.trBase, self->activator, 200, (int)(float)t * RAILGUN_BOOMSPHERE_MAXRADIUS / (RAILGUN_IMPLODE_STARTTIME - RAILGUN_EXPLODE_STARTTIME));
	}
