...
//	wc.hIcon = LoadIcon(s_hInstance, MAKEINTRESOURCE(IDI_ICON1));
	...
	s_wcd.hWnd = CreateWindowEx(0, DEDCLASS, "XreaL Console", DEDSTYLE, (swidth - 600) / 2, (sheight - 450) / 2, rect.right - rect.left + 1, rect.bottom - rect.top + 1, NULL, NULL, s_hInstance, NULL);
...
#if defined(INPUTBOX)
	s_wcd.hwndInputLine = CreateWindow("edit", NULL, WS_CHILD|WS_VISIBLE|WS_BORDER|ES_LEFT|ES_AUTOHSCROLL, 6, 400, 528, 20, s_wcd.hWnd, (HMENU)INPUT_ID, s_hInstance, NULL); // child window ID
#endif
	// create the buttons
	s_wcd.hwndButtonCopy = CreateWindow("button", NULL, BS_PUSHBUTTON|WS_VISIBLE|WS_CHILD|BS_DEFPUSHBUTTON, 5, 425, 72, 24, s_wcd.hWnd, (HMENU)COPY_ID, s_hInstance, NULL); // child window ID

	SendMessage(s_wcd.hwndButtonCopy, WM_SETTEXT, 0, (LPARAM)"copy");

	s_wcd.hwndButtonClear = CreateWindow("button", NULL, BS_PUSHBUTTON|WS_VISIBLE|WS_CHILD|BS_DEFPUSHBUTTON, 82, 425, 72, 24, s_wcd.hWnd, (HMENU)CLEAR_ID, s_hInstance, NULL); // child window ID

	SendMessage(s_wcd.hwndButtonClear, WM_SETTEXT, 0, (LPARAM)"clear");

	s_wcd.hwndButtonQuit = CreateWindow("button", NULL, BS_PUSHBUTTON|WS_VISIBLE|WS_CHILD|BS_DEFPUSHBUTTON, 462, 425, 72, 24, s_wcd.hWnd, (HMENU)QUIT_ID, s_hInstance, NULL); // child window ID

	SendMessage(s_wcd.hwndButtonQuit, WM_SETTEXT, 0, (LPARAM)"quit");
	// create the scrollbuffer
	s_wcd.hwndBuffer = CreateWindow("edit", NULL, WS_CHILD|WS_VISIBLE|WS_VSCROLL|WS_BORDER|ES_LEFT|ES_MULTILINE|ES_AUTOVSCROLL|ES_READONLY, 6, 40, 526, 354, s_wcd.hWnd, (HMENU)EDIT_ID, s_hInstance, NULL); // child window ID
...
void CON_SetErrorText(const char *buf) {
	...
	if (!s_wcd.hwndErrorBox) {
		s_wcd.hwndErrorBox = CreateWindow("static", NULL, WS_CHILD|WS_VISIBLE|SS_SUNKEN, 6, 5, 526, 30, s_wcd.hWnd, (HMENU)ERRORBOX_ID, s_hInstance, NULL); // child window ID
