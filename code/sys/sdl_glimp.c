		...
		*/
		...
		if (wglMakeCurrent(opengl_context.hDC, opengl_context.hGLRC)) {
			...
			ri.Error(ERR_FATAL, "Could not initialize OpenGL %i.%i context\n"
								"Make sure your graphics card supports OpenGL %i.%i or newer", r_glMinMajorVersion->integer, r_glMinMinorVersion->integer, r_glMinMajorVersion->integer, r_glMinMinorVersion->integer);
		}
	}
...
static int GLimp_SetMode(int mode, qboolean fullscreen, qboolean noborder) {
	...
	if (videoInfo == NULL) {
		...
		if (videoInfo->current_h > 0) {
			// Guess the display aspect ratio through the desktop resolution by assuming (relatively safely) that it is set at or close to
