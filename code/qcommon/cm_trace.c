...
void CM_TransformedBoxTrace(trace_t *results, const vec3_t start, const vec3_t end, vec3_t mins, vec3_t maxs, clipHandle_t model, int brushmask, const vec3_t origin, const vec3_t angles, traceType_t type) {
	...
	*results = trace;
	...
	// adjust so that mins and maxs are always symmetric, which avoids some complications with plane expanding of rotated bmodels
	for (i = 0; i < 3; i++) {
		offset[i] = (mins[i] + maxs[i]) * 0.5;
		symetricSize[0][i] = mins[i] - offset[i];
		symetricSize[1][i] = maxs[i] - offset[i];
	...
	// if the bmodel was rotated and there was a collision
	if (rotated && trace.fraction != 1.0) {
		// Tr3B: we rotated our trace into the space of the clipModel so we have to rotate the trace plane normal back to world space
