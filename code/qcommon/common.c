	...
	while (1) {
		...
		switch (ev.evType) {
			...
			case SE_CONSOLE:
				...
				break;
			case SE_PACKET:
				// this cvar allows simulation of connections that drop a lot of packets.  Note that loopback connections don't go through here at all.
				...
				// we must copy the contents of the message out, because the event buffers are only large enough to hold the
				// exact payload, but channel messages need to be large enough to hold fragment reassembly
