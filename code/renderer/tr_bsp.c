...
static void R_LoadLightmaps(lump_t *l, const char *bspName) {
	...
	if (!len) {
		...
		if (tr.worldHDR_RGBE) {
			...
			if (r_hdrRendering->integer && r_hdrLightmap->integer && glConfig2.framebufferObjectAvailable && glConfig2.framebufferBlitAvailable && glConfig2.textureFloatAvailable && glConfig2.textureHalfFloatAvailable) {
				int width, height;
...
Vertices will be joined at the patch side a crack is first found, at the other side of the patch (on the same row or column) the
...
/*static void OptimizeTriangles(int numVerts, srfVert_t *verts, int numTriangles, srfTriangle_t *triangles, qboolean(*compareVert)(const srfVert_t *v1, const srfVert_t *v2)) {
#if 1
	srfTriangle_t *tri;
	int i, j, k, l;
	static int redundantIndex[MAX_MAP_DRAW_VERTS];
...
static void BuildRedundantIndices(int numVerts, const srfVert_t *verts, int *redundantIndex, qboolean(*CompareVert)(const srfVert_t *v1, const srfVert_t *v2)) {
	int i, j;
...
//	trRefLight_t *light;
	...
	if (c_redundantVertexes) {
		ri.Printf(PRINT_DEVELOPER,"...removed %i redundant vertices from staticWorldMesh %i(%s, %i verts %i tris)\n", c_redundantVertexes, vboSurfaces.currentElements, shader->name, numVerts, numTriangles);
	}

	s_worldData.vbo = R_CreateVBO2(va("bspModelMesh_vertices %i", 0), numVerts, optimizedVerts, ATTR_POSITION|ATTR_TEXCOORD|ATTR_LIGHTCOORD|ATTR_TANGENT|ATTR_BINORMAL|ATTR_NORMAL|ATTR_COLOR|GLCS_LIGHTCOLOR|ATTR_LIGHTDIRECTION);
#else
	...
	for (m = 1, model = s_worldData.models; m < s_worldData.numModels; m++, model++) {
		...
		for (k = 0; k < numSurfaces; k++) {
			...
			if (shader != oldShader || (r_precomputedLighting->integer ? lightmapNum != oldLightmapNum : 0)) {
				...
				if (c_redundantVertexes) {
					ri.Printf(PRINT_DEVELOPER, "...removed %i redundant vertices from staticEntityMesh %i(%s, %i verts %i tris)\n", c_redundantVertexes, vboSurfaces.currentElements, shader->name, numVerts, numTriangles);
				}

				vboSurf->vbo = R_CreateVBO2(va("staticBspModel%i_VBO %i", m, vboSurfaces.currentElements), numVerts, optimizedVerts, ATTR_POSITION|ATTR_TEXCOORD|ATTR_LIGHTCOORD|ATTR_TANGENT|ATTR_BINORMAL|ATTR_NORMAL|ATTR_COLOR|GLCS_LIGHTCOLOR|ATTR_LIGHTDIRECTION, VBO_USAGE_STATIC);
#else
				vboSurf->vbo = R_CreateVBO2(va("staticBspModel%i_VBO %i", m, vboSurfaces.currentElements), numVerts, verts, ATTR_POSITION|ATTR_TEXCOORD|ATTR_LIGHTCOORD|ATTR_TANGENT|ATTR_BINORMAL|ATTR_NORMAL|ATTR_COLOR
#if !defined(COMPAT_Q3A) && !defined(COMPAT_ET)
								|ATTR_PAINTCOLOR|ATTR_LIGHTDIRECTION
#endif
								, VBO_USAGE_STATIC);
		...
		for (k = 0; k < numCaches; k++) {
			...
			if (alphaTest ? shader != oldShader : alphaTest != oldAlphaTest) {
				...
				if (alphaTest) {
					...
					if (c_redundantVertexes) {
						ri.Printf(PRINT_DEVELOPER, "...removed %i redundant vertices from staticShadowPyramidMesh %i(%s, %i verts %i tris)\n", c_redundantVertexes, c_vboShadowSurfaces, shader->name, numVerts, numTriangles);
					}
#endif
					vboSurf->vbo = s_worldData.vbo;
...
#if 0
		ri.Printf(PRINT_ALL, "light %i: origin(%i %i %i)radius(%i %i %i)color(%f %f %f)\n", i, (int)light->l.origin[0], (int)light->l.origin[1], (int)light->l.origin[2], (int)light->l.radius[0], (int)light->l.radius[1], (int)light->l.radius[2], light->l.color[0], light->l.color[1], light->l.color[2]);
#endif
...
#else
		if ((fabs(xyz[0] - vertexHash->vcd.xyz[0])) > HASH_XYZ_EPSILON || (fabs(xyz[1] - vertexHash->vcd.xyz[1])) > HASH_XYZ_EPSILON || (fabs(xyz[2] - vertexHash->vcd.xyz[2])) > HASH_XYZ_EPSILON) {
			continue;
		}
#endif
		return vertexHash;
...
//	fileBuf = ri.Z_Malloc(REF_CUBEMAP_STORE_SIZE * REF_CUBEMAP_STORE_SIZE * 4);
	...
	for (j = 0; j < tr.cubeProbes.currentElements; j++) {
		...
		//ri.Printf(PRINT_ALL, "rendering cubemap at(%i %i %i)\n", (int)cubeProbe->origin[0], (int)cubeProbe->origin[1], (int)cubeProbe->origin[2]);
		...
		for (i = 0; i < 6; i++) {
			...
			if (qfalse) {
				...
				R_SubImageCpy(fileBuf, fileBufX * REF_CUBEMAP_SIZE, fileBufY * REF_CUBEMAP_SIZE, REF_CUBEMAP_STORE_SIZE, REF_CUBEMAP_STORE_SIZE, tr.cubeTemp[i], REF_CUBEMAP_SIZE, REF_CUBEMAP_SIZE, 4, qtrue);
...
void RE_LoadWorldMap(const char *name) {
	...
	// TODO: this is sort of silly. I'm going to do a general cleanup on fog stuff now that I can see how it's been used (functionality can narrow since it's not used as much as it's designed for).
#if defined(COMPAT_ET)
...
//	ri.Cmd_ExecuteText(EXEC_NOW, "updatescreen\n");
	...
	R_BuildCubeMaps();
	// never move this to RE_BeginFrame because we need it to set it here for the first frame but we need the information across 2 frames
