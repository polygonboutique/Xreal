...
Generates a texture projection matrix for a triangle. Returns qfalse if a texture matrix cannot be created.
=======================================================================================================================================
*/
...
Projected polygons should be 3 or 4 points. If a single point is passed in (numPoints == 1) then the decal will be omnidirectional.
Omnidirectional decals use points[0] as center and projection[3] as radius pass in lifeTime < 0 for a temporary mark.
=======================================================================================================================================
*/
...
qboolean R_TestDecalBoundingBox(decalProjector_t *dp, vec3_t mins, vec3_t maxs) {
	...
	if (mins[0] >= (dp->center[0] + dp->radius) || maxs[0] <= (dp->center[0] - dp->radius) || mins[1] >= (dp->center[1] + dp->radius) || maxs[1] <= (dp->center[1] - dp->radius) || mins[2] >= (dp->center[2] + dp->radius) || maxs[2] <= (dp->center[2] - dp->radius)) {
		return qfalse;
	}
