...
void R_AddBrushModelInteractions(trRefEntity_t *ent, trRefLight_t *light) {
	...
	// cull the entire model if it is outside the view frustum and we don't care about proper shadowing
...
static void R_SetupEntityLightingGrid(trRefEntity_t *ent, vec3_t forcedOrigin) {
	...
	if (forcedOrigin) {
		...
		if (ent->e.renderfx & RF_LIGHTING_ORIGIN) {
			// seperate lightOrigins are needed so an object that is sinking into the ground can still be lit, and so
...
void R_SetupLightDepthBounds(trRefLight_t *light) {
	...
	if (r_shadows->integer == SHADOWING_STENCIL && glDepthBoundsEXT) {
		...
		for (j = 0; j < 8; j++) {
			...
			if (window[0] < 0 || window[0] >= tr.viewParms.viewportWidth || window[1] < 0 || window[1] >= tr.viewParms.viewportHeight) {
