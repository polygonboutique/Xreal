...
void SV_WriteDownloadToClient(client_t *cl, msg_t *msg) {
	...
	// loop up to window size times based on how many blocks we can fit in the client snapMsec and rate
	// based on the rate, how many bytes can we fit in the snapMsec time of the client normal rate/snapshotMsec calculation
	rate = cl->rate;
...
void SV_WriteVoipToClient(client_t *cl, msg_t *msg) {
	...
	for (i = 0; i < cl->queuedVoipPackets; i++, packet++) {
		...
		// you have to start with a svc_EOF, so legacy clients drop the rest of this packet. Otherwise, those without VoIP support will
		// see the svc_voip command, then panic and disconnect.
		// generally we don't send VoIP packets to legacy clients, but this serves as both a safety measure and a means to keep demo files compatible.
		MSG_WriteByte(msg, svc_EOF);
...
static void SV_UserVoip(client_t *cl, msg_t *msg) {
	...
	// (the three recip * values are 31 bits each(ignores sign bit so we can get a -1 error from MSG_ReadLong()...), allowing for 93 clients.)
...
void SV_ExecuteClientMessage(client_t *cl, msg_t *msg) {
	...
	do {
		c = MSG_ReadByte(msg);
		// see if this is an extension command after the EOF, which means we got data that a legacy server should ignore.
