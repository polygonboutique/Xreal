...
void CL_RequestMotd(void) {
	...
	// NOTE: the Com_Milliseconds xoring only affects the lower 16-bit word, but I decided it was enough randomization
...
void CL_CheckForResend(void) {
	...
	switch (cls.state) {
		...
		case CA_CHALLENGING:
			...
			strcpy(data, "connect ");
			// TTimo adding " " around the userinfo string to avoid truncated userinfo on the server (Com_TokenizeString tokenizes around spaces)
			data[8] = '"';
...
void CL_DisconnectPacket(netadr_t from) {
	...
	// if we have received packets within three seconds, ignore it (it might be a malicious spoof)
...
void CL_Frame(int msec) {
	...
	if (clc.downloadCURLM) {
		...
		// we can't process frames normally when in disconnected download mode since the ui vm expects cls.state to be CA_CONNECTED
		if (clc.cURLDisconnected) {
...
void CL_GlobalServers_f(void) {
	...
	if (to.type == NA_IP6 || to.type == NA_MULTICAST6) {
		...
		// TODO: test if we only have an IPv6 connection. If it's the case, request IPv6 servers only by appending " ipv6" to the command
	} else {
		cmdname = "getservers";
	}
