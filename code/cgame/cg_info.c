...
void CG_DrawInformation(void) {
	...
	if (value) {
		CG_Text_PaintAligned(x, y, va("Timelimit %i", value), 0.2f, UI_DROPSHADOW, text_color_normal, &cgs.media.freeSansBoldFont);
		y += y_offset;
	}
	...
	if (cgs.gametype < GT_CTF) {
		...
		if (value) {
			CG_Text_PaintAligned(x, y, va("Fraglimit %i", value), 0.2f, UI_DROPSHADOW, text_color_normal, &cgs.media.freeSansBoldFont);
		...
		if (value) {
			CG_Text_PaintAligned(x, y, va("Capturelimit %i", value), 0.2f, UI_DROPSHADOW, text_color_normal, &cgs.media.freeSansBoldFont);
			y += y_offset;
		}
	}
}
