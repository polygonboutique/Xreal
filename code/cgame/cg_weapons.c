...
static void CG_LightningBolt(centity_t *cent, vec3_t origin) {
	...
	if (target >= 0 && target < ENTITYNUM_MAX_NORMAL) {
		...
		AddDischargeFlash(start, cent->lerpAngles, &cent->gunFlash1, cent->currentState.number, vec3_origin, vec3_origin, cgs.media.dischargeFlashShader);

		VectorMA(origin, -1.75, right, start);
		AddDischargeFlash(start, cent->lerpAngles, &cent->gunFlash2, cent->currentState.number, vec3_origin, vec3_origin, cgs.media.dischargeFlashShader);
	}
...
void CG_AddPlayerWeapon(refEntity_t *parent, playerState_t *ps, centity_t *cent, int team) {
	...
	// if the index of the nonPredictedCent is not the same as the clientNum then this is a fake player(like on teh single player podiums), so
...
void CG_AddViewWeapon(playerState_t *ps) {
	...
	if (weapon->viewModel && weapon->viewModel_animations[WEAPON_READY].handle) {
		...
		// then this is a fake player(like on teh single player podiums), so o ahead and use the cent
		if ((nonPredictedCent - cg_entities) != cent->currentState.clientNum) {
			nonPredictedCent = cent;
		}
