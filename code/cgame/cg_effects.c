...
void CG_ExplosiveExplode(centity_t *cent) {
	...
	switch (cent->currentState.generic1) {// Type
		...
		case ENTMAT_BODY:
			// Blood
			CG_ExplosiveBlood(cent->lerpOrigin, mins, maxs, (3 * cent->currentState.torsoAnim + 2 * cent->currentState.legsAnim + cent->currentState.weapon));
			break;
		case ENTMAT_BRICK:
		case ENTMAT_STONE:
		case ENTMAT_TILES:
			// Dust
			CG_ExplosiveDust(cent->lerpOrigin, mins, maxs, (2 * cent->currentState.torsoAnim + cent->currentState.legsAnim), (10 * cent->currentState.torsoAnim + 5 * cent->currentState.legsAnim + 2 * cent->currentState.weapon));
			break;
		case ENTMAT_PLASTER:
			// Dust & flakes?
			CG_ExplosivePlaster(cent->lerpOrigin, mins, maxs, (8 * cent->currentState.torsoAnim + 4 * cent->currentState.legsAnim + 1 * cent->currentState.weapon));
			break;
		case ENTMAT_FIBERS:
			// Fiber flakes?
			break;
		...
		case ENTMAT_SMOKE:
			...
			CG_ExplosiveSmoke(cent->lerpOrigin, mins, maxs, (4 * cent->currentState.torsoAnim + 2 * cent->currentState.legsAnim + 1 * cent->currentState.weapon));
			break;
		case ENTMAT_GAS:
			// Gas (heavier than air)
			CG_ExplosiveGas(cent->lerpOrigin, mins, maxs, (4 * cent->currentState.torsoAnim + 2 * cent->currentState.legsAnim + 1 * cent->currentState.weapon));
			break;
		case ENTMAT_FIRE:
			// Flames
			CG_ExplosiveFire(cent->lerpOrigin, mins, maxs, (9 * cent->currentState.torsoAnim + 5 * cent->currentState.legsAnim + 3 * cent->currentState.weapon));
			break;
		case ENTMAT_NONE:
		default:
			break;
