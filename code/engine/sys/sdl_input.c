...
static void IN_InitJoystick(void) {
	...
	if (!Q_stricmp(SDL_JoystickName(in_joystickNo->integer), "Microsoft X - Box 360 pad") || !Q_stricmp(SDL_JoystickName(in_joystickNo->integer), "Controller(XBOX 360 For Windows)")) {
...
static void IN_Xbox360ControllerMove(void) {
	...
	if (hat != stick_state.oldhats) {
		if (hat != stick_state.oldhats) {
			int key;
			const int allHatDirections = (SDL_HAT_UP|SDL_HAT_RIGHT|SDL_HAT_DOWN|SDL_HAT_LEFT);
