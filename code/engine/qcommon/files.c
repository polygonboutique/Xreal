...
static void FS_CopyFile(char *fromOSPath, char *toOSPath) {
	...
	len = ftell(f);
	fseek(f, 0, SEEK_SET);
	// we are using direct malloc instead of Z_Malloc here, so it probably won't work on a mac... Its only for developers anyway...
	buf = malloc(len);
			...
			do {
				...
				if (!FS_FilenameCompare(pakFile->name, filename)) {
					...
					if (!(pak->referenced & FS_GENERAL_REF)) {
						if (!FS_IsExt(filename, ".mtr", len) && !FS_IsExt(filename, ".txt", len) && !FS_IsExt(filename, ".cfg", len) && !FS_IsExt(filename, ".config", len) && !FS_IsExt(filename, ".bot", len) && !FS_IsExt(filename, ".arena", len) && !FS_IsExt(filename, ".menu", len) && !strstr(filename, "levelshots")) {
